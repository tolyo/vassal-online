...
public class GamePieceLayout extends AbstractConfigurable implements Visualizable {
  ...
  public Class<?>[] getAttributeTypes() {
    return new Class<?>[] {
      String.class, Integer.class, Integer.class, BorderConfig.class, LayoutConfig.class
    };
  }
  ...
  public Image buildImage(GamePieceImage defn) {
    ...
    final BufferedImage image =
        ImageUtils.createCompatibleTranslucentImage(Math.max(width, 1), Math.max(height, 1));
    ...
    if (getBorder().equals(BORDER_3D)) {
      ...
      if (getBorder().equals(BORDER_PLAIN) || getBorder().equals(BORDER_FANCY)) {
        ...
        if (getBorder().equals(BORDER_FANCY)) {
          final Color lt = new Color(bg.getRed() / 2, bg.getGreen() / 2, bg.getBlue() / 2);
          final Color dk =
              new Color(
                  bg.getRed() + (255 - bg.getRed()) / 2,
