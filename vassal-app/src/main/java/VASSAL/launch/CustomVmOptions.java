...
public class CustomVmOptions {
  ...
  private void createInitialVmOptionsFile(File confDir) {
    ...
    try (InputStream is = classLoader.getResourceAsStream(VM_OPTIONS_FILE_NAME)) {
      if (is == null) {
        log.error(
            "Template for custom VM options "
                + VM_OPTIONS_FILE_NAME
                + " not found in the Vassal distribution"); // NON-NLS
        return;
      }
      Files.copy(is, Path.of(confDir.getAbsolutePath(), VM_OPTIONS_FILE_NAME));
    } catch (IOException e) {
      log.error(
          "Unable to copy " + VM_OPTIONS_FILE_NAME + " to " + confDir.getAbsolutePath(),
          e); // NON-NLS
    }
  }
