...
public class TranslateWindow extends JDialog
  ...
  public TranslateWindow(
      Frame owner, boolean modal, final Translatable target, ConfigureTree tree) {
    super(owner, modal);
    this.target = target;
  ...
  protected void initComponents() {
    ...
    setTitle(
        Resources.getString(
            "Editor.TranslateWindow.translate",
            ConfigureTree.getConfigureName((Configurable) target)));
  ...
  protected Component getHeaderPanel() {
    ...
    final JButton addButton =
        new JButton(Resources.getString("Editor.TranslateWindow.add_translation"));
  ...
  /**
   * User has clicked on the Add Translation button. Create a new PropertiesWindow for a translation
   * and display it.
   */
  protected void getNewTranslation() {
    final Translation t = new Translation();
    final PropertiesWindow w =
        new MyPropertiesWindow(
            (Frame) SwingUtilities.getAncestorOfClass(Frame.class, this), false, t, null, this);
    w.setVisible(true);
  }
  ...
  protected void refreshTranslationList(Configurable target) {
    ...
    if (list != null) {
      if (list.get(0).contains(newTranslation)) {
        WarningDialog.show(
            "Editor.TranslateWindow.translation_exists", newTranslation.getDescription());
        return;
      }
  ...
  protected Component getButtonPanel() {
    ...
    helpButton.addActionListener(
        new ShowHelpAction(
            HelpFile.getReferenceManualPage("Translations.html", "module").getContents(),
            null)); // NON-NLS
    ...
    cancelButton = new JButton(Resources.getString(Resources.CANCEL));
  ...
  protected void commitTableEdit() {
    if (keyTable != null && keyTable.isEditing()) {
      ...
      if (row != -1 && column != -1) keyTable.editCellAt(row, column);
    }
  }
  ...
  /**
   * When a key is selected in the table, display the source and translated texts in the right hand
   * panels
   */
  @Override
  ...
  protected boolean querySave() throws IOException {
    switch (JOptionPane.showConfirmDialog(
        this,
        Resources.getString("Editor.TranslateWindow.save_now"),
        Resources.getString("Editor.TranslateWindow.unsaved"),
  ...
  protected static class JComponentCellEditor
    ...
    public boolean shouldSelectCell(EventObject anEvent) {
      if (editorComponent != null
        ...
        final Component dispatchComponent =
  ...
  class MyTableModel extends AbstractTableModel {
    ...
    public void update() {
      ...
      keyTable
          .getColumnModel()
          .getColumn(ATTR_COL)
          .setCellRenderer(new MyTableCellRenderer(keyTarget));
      keyTable.getColumnModel().getColumn(CC_COL).setMaxWidth(25);
    }
  }

  /** Custom table cell renderer - Change color of key names based on translation status */
  class MyTableCellRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      final String translation =
          currentTranslation == null ? "" : currentTranslation.translate(fullKey);
        ...
        final String t =
            currentTranslation == null
                ...
                : currentTranslation.translate(
                    keyTarget.getI18nData().getFullPrefix() + keys[row]); // $NON-NLS-1$
  ...
  /**
   * Custom tree cell renderer - Change color of component names based on translation status of
   * children
   */
