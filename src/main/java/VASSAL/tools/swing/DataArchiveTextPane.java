...
/**
 * Extends JTextPane in a way that lets us put an HTML-compliant string that might access images
 * from the vmod archive
 *
 * <p>Essentially behaves like a JLabel on steroids, the key thing being that <img> links in the
 * HTML can access images in the module.
 */
public class DataArchiveTextPane extends JTextPane {
  ...
  public DataArchiveTextPane(String text, String styleName, Color color, Font font) {
    ...
    final LabelerDataArchiveHTMLEditorKit kit =
        new LabelerDataArchiveHTMLEditorKit(GameModule.getGameModule().getDataArchive());
    setEditorKit(kit);
    ...
    // Remove original HTML markers if present and replace wrapping a styled div, inserting into
    // document
    ...
    try {
      kit.insertHTML(
          doc,
          doc.getLength(),
          "<html><div class=\"" + styleName + "\">" + fixed + "</div></html>",
          0,
          0,
          null); // NON-NLS
