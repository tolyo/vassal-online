...
/**
 * PieceMover handles the "Drag and Drop" of pieces and stacks, onto or within a Map window. It
 * implements MouseListener and handles dragging and dropping of both individual pieces, stacks, and
 * groups of pieces/stacks. It is a subcomponent of Map. <br>
 * <br>
 * For the selection/deselection of pieces and band-selecting pieces by "dragging a lasso around
 * them", see {@link KeyBufferer}.
 */
public class PieceMover extends AbstractBuildable
  ...
  protected GamePiece
      dragging; // Anchor piece that we're dragging (along with everything else in DragBuffer)

  // PieceMover provides the "clear move history" button for Map - the configurer dialog is found
  // there.
  ...
  protected LoadedCargoDropTargetSelector
      loadedCargoDropTargetSelector; // Specialised for cargo loaded on a mat
  protected MatDropTargetSelector matDropTargetSelector; // Specialised for mats
  protected PieceVisitorDispatcher
      selectionProcessor; // Processes drag target after having been selected
  ...
  /**
   * Adds this component to its parent map. Add ourselves as a mouse listener, drag gesture
   * listener, etc.
   *
  ...
  public void addTo(Buildable b) {
    ...
    dropTargetSelector =
        createDropTargetSelector(); // Obsolete from 3.6, but maintains backwards-compatibility with
    // e.g. VASL
    ...
    // Because of the strange legacy scheme of halfway-running a Toolbar button "on behalf of Map",
    // we have to set some its attributes
    ...
    setAttribute(Map.MARK_UNMOVED_HOTKEY, map.getAttributeValueString(Map.MARK_UNMOVED_HOTKEY));
    setAttribute(Map.MARK_UNMOVED_REPORT, map.getAttributeValueString(Map.MARK_UNMOVED_REPORT));
  }

  /**
   * Creates a {@link MovementReporter} for a collection of commands containing AddPiece and
   * MovePiece commands, which will supply auto-report messages corresponding to those commands
   *
   ...
   *
   * <p>When the user *completes* a drag-and-drop operation, the pieces being dragged will either
   * be: <br>
   * (1) combined in a stack with an existing piece (or stack, or deck) on the map <br>
   * (2) placed alone in a brand new stack <br>
   * (3) or, in the case of non-stacking pieces, placed on the map without stack. <br>
   * <br>
   * For <i>each</i> of the stacks/pieces being dragged, we need to determine which of those
   * outcomes applies. This "drop target selector" will be fed, one by one, all of the pieces on the
   * map, and for each one must check whether it would make a valid "merge target" for the current
   * dragged piece under consideration (the PieceFinder's {@link PieceFinder#select} method provides
   * the map and location, and the PieceMover's {@link PieceMover#dragging} field tells the "dragged
   * piece under consideration". Each method is to returns the target piece/stack/deck to merge with
   * if it has been passed a valid target, or null if it is not a valid match. <br>
   * <br>
   * The Map's {@link Map#findAnyPiece(Point, PieceFinder)} method, which feeds this, will be
   * responsible for iterating through the list of possible pieces in proper visual order so that we
   * check the pieces "visually on top" first. <br>
   * <br>
   *
  ...
  private class StandardDropTargetSelector extends PieceFinder.Movable {
    ...
    /**
     * When an unstacked piece exists on the map, we see if this is a piece we could form a stack
     * with -- if it is at our precise location (or the location we would be getting snapped to). We
     * must also check if stacking is enabled and if both the target "piece" and the "dragging"
     * piece fit all the usual "can merge" requirements (both are "stacking" pieces, and in the same
     * visual layer)
     *
    ...
    public Object visitDefault(GamePiece piece) {
      ...
      if (this.map.getStackMetrics().isStackingEnabled()
        ...
        if (this.map.isLocationRestricted(pt)) {
          ...
          // The "super" visitDefault checks if our point is inside the visual boundaries of the
          // piece.
      ...
      if (selected != null
          ...
          && selected.getParent().topPiece()
              == selected) { // NOTE: topPiece() returns the top VISIBLE piece (not hidden by
        // Invisible trait)
        selected = null;
      }
    ...
    public Object visitStack(Stack s) {
      ...
      if (this.map.getStackMetrics().isStackingEnabled()
          ...
          && !DragBuffer.getBuffer().containsAllMembers(s)
          && // BR// Don't merge back into a stack we are in the act of emptying
          s.topPiece()
              != null) { // NOTE: topPiece() returns the top VISIBLE piece (not hidden by Invisible
        // trait)

        // If the target point is on a snappable grid and the target stack is not expanded, then
        // snap the point
        // and see if it aligns with the Stack location
        // Exceptions: 1. Cargo loaded on a Mat can stack off-grid
        //             2. 'Ignore Grid' pieces can now stack normally, so can create valid off-grid
        // stacks
        final boolean stackIgnoresGrid =
            isStackLoadedCargo(s)
                || Boolean.TRUE.equals(s.getPieceAt(0).getProperty(Properties.IGNORE_GRID));
        ...
        if (this.map.isLocationRestricted(pt) && !s.isExpanded() && !stackIgnoresGrid) {
          ...
          // The super's visitStack handles checking to see if our point is inside the visual
          // "shape" of the stack
  ...
  /**
   * A DropTargetSeletor for Cargo that is loaded onto a Mat. We are only interested in stacking
   * with other Counters also loaded on the same mat.
   */
  private class LoadedCargoDropTargetSelector extends StandardDropTargetSelector {
    ...
    /**
     * Only interested in stacking with another Stack if it has at least one piece and the pieces
     * are on this mat
     *
  ...
  /** DropTargetSelector for moving mats. Mats holding cargo must never enter a Deck */
  ...
  /**
   * When the user *starts* a potential drag-and-drop operation by clicking on the map, a piece from
   * the map is selected by the dragTargetSelector. What happens to that piece is determined by the
   * {@link PieceVisitorDispatcher} instance returned by this method. The default implementation
   * does the following: <br>
   * (1) If a Deck, add the (single) top piece to the drag buffer <br>
   * (2) If a stack, add it to the drag buffer. If the stack is a member of the "currently selected
   * pieces" (i.e. {@link KeyBuffer}), then add any other already-selected pieces and stacks to the
   * drag buffer as well. <br>
  ...
  protected PieceVisitorDispatcher createSelectionProcessor() {
    return new DeckVisitorDispatcher(
        new DeckVisitor() {
          /**
           * We've picked a Deck - Clear the drag buffer and add Deck's top piece to the drag
           * buffer.
           *
          ...
          /**
           * We've picked a Stack. Clear the drag buffer and add the stack's pieces to it. If the
           * stack is part of the "currently selected pieces" (i.e. {@link KeyBuffer}), then add any
           * other pieces/stacks as well.
           *
          ...
          public Object visitStack(Stack s) {
            ...
            final boolean selectAllUnitsInStackRegardlessOfSelection =
                (Boolean)
                    GameModule.getGameModule().getPrefs().getValue(Map.MOVING_STACKS_PICKUP_UNITS);
          ...
          public Object visitDefault(GamePiece selected) {
            ...
            if (GameModule.getGameModule().isMatSupport()) {
              ...
              for (final GamePiece piece : checkDrag) {
                final List<GamePiece> cargoList =
                    (List<GamePiece>) piece.getProperty(Mat.MAT_CONTENTS);
                if (cargoList != null) {
  ...
  /**
   * Returns the {@link PieceFinder} instance that will select a {@link GamePiece} for processing
   * when the user clicks on the map. The default implementation is to return the first piece whose
   * shape contains the point clicked on.
   *
   * @return Piece Finder
   */
  ...
  protected void initButton() {
    ...
    if (!GlobalOptions.NEVER.equals(value)) {
      if (markUnmovedButton == null) {
            ...
            e -> {
              ...
              if ((markUnmovedReport != null) && !markUnmovedReport.isEmpty()) {
                ...
                final String reportText =
                    format.getLocalizedText(map, this, "Editor.Map.mark_unmoved_button_report");
        ...
        markUnmovedButton.setAttribute(
            Map.MARK_UNMOVED_HOTKEY, map.getAttributeValueString(Map.MARK_UNMOVED_HOTKEY));
  ...
  /**
   * Invoked just BEFORE a piece is moved. Sets the "OldLocations" properties for the piece. Marks
   * the piece as "moved" if it has changed positions, and removes the piece from its old stack, if
   * any.
   *
   * @return Command encapsulating anything this method did, for replay in log file or on other
   *     clients
   */
  protected Command movedPiece(GamePiece p, Point loc) {
    // Make sure we don't end up sending I'm-empty commands from decks that were emptied by other
    // players (e.g. online or log-step)
  ...
  /**
   * Populates the "OldLocations" properties (e.g. OldMap, OldZone, etc) for the piece (or for a
   * stack, for all the pieces contained in it), based on their present locations, in preparation
   * for moving them to a new location.
   *
   ...
   * @param locDefinitelyChanged true if piece definitely changed locations or mats (for possibly
    ...
    if (!hasMoved || shouldMarkMoved()) {
      if (p instanceof Stack) {
        ...
        if (p.getId() != null) {
          ...
          p.setProperty(
              (!hasMoved || locDefinitelyChanged) ? Properties.MOVED : Properties.MAYBE_MOVED,
  ...
  /**
   * Checks Global Options settings (and if necessary, the user preference) about whether we mark
   * moved pieces as "moved" or not.
   *
  ...
  /**
   * This is the key method for handling the "Drop" part of Drag and Drop. <br>
   * (1) Moves each piece in the {@link DragBuffer} to its proper destination, based on the mouse
   * having been released at point "p" (if multiple pieces were being dragged after being e.g.
   * band-selected, each individual piece's destination point will vary with the piece's offset from
   * the anhor point that started the drag). This also involves removing each piece from any
   * stacks/decks it was part of, and possibly removing it from its old map if it is changing maps.
   * <br>
   * (2) As each piece is moved, finds appropriate "merge targets" (pieces that should be combined
   * with it in a stack) if they exist, or forms new stacks where needed. Adds the piece to old or
   * new stacks as appropriate, or directly to the map if non-stacking. If the piece has been moved
   * between maps (or onto a map for the first time), the piece is also added to the map's piece
   * collection. <br>
   * (3) If auto-reporting of moves is enabled, creates the report. <br>
   * (4) Applies any apply-on-move keystroke (from the "Key command to apply to all units ending
   * movement on this map" field of the map) to each piece, as appropriate. <br>
   * (5) Returns a command to encapsulate any and all changes made, for replay in log file or on
   * other clients
   *
   * @param map Map
  ...
  public Command movePieces(Map map, Point p) {
    ...
    // This will be a list of every piece we end up moving somewhere, and thus the list of pieces to
    // receive "apply-on-move" key command
    ...
    for (final GamePiece gp : newDragBuffer) {
      ...
      // Find a piece (already moved in the same drag operation) that we can merge with at the
      // destination point
      ...
      // If we're not forming a stack with one of our pieces that just moved, we now look for an
      // already-existing
      // piece at the destination point
      if (mergeWith == null) {
        // This will now process through every "piece" (including stacks & decks) on the map,
        // feeding each to the
        // dropTargetSelector (see our createDropTargetSelector method above). The
        // dropTargetSelector will return
        // non-null if it finds a piece (including stack or deck) at this piece's drop location that
        // is suitable to
        // merge with/into.
        mergeWith = map.findAnyPiece(p, getDropTargetSelector(dragging, currentCargo, currentMat));

        // If we get here with no merge target, we know we'll either be starting a new stack or
        // putting the
        ...
        if (mergeWith == null) {
          ...
          if (currentCargo == null) {
            ...
            // Snapping behaviour of cargo depends on whether or not there is a valid mat at the
            // destination
            // and whether or not we are moving with a Mat
            if (currentMat == null && currentCargo.locateNewMat(map, p) == null) {
              // Not moving with a Mat and there is no valid mat at the destination. If this cargo
              // is coming from a mat,
          ...
          if (!ignoreGrid) {
            ...
            // Now that we've snapped to the grid, perhaps there is a piece here that we can merge
            // with.
            mergeWith =
                map.findAnyPiece(p, getDropTargetSelector(dragging, currentCargo, currentMat));
          }
        }

        offset = new Point(p.x - dragging.getPosition().x, p.y - dragging.getPosition().y);

        // If we've HAVE found a piece to merge with, and we're going to a map that allows stacking,
        // then
      ...
      if (mergeWith == null) {
        ...
        comm =
            comm.append(
                map.placeAt(
                    dragging, p)); // Adds piece to destination map (removes from old map/stack)
        ...
        if (!(dragging instanceof Stack)
          ...
          if (parent != null) {
            ...
            // BR// We've made a new stack, so put it on the list of merge targets, in case more
            // pieces land here too
            mergeCandidates = new ArrayList<>();
            mergeCandidates.add(dragging);
        ...
        // If we get here, we DID find a piece (possibly Deck or Stack) to merge with/into, so we
        // handle that.

        // Do not add pieces to a Deck that are Obscured to us, or that the Deck does not want to
        // contain.
        // Removing them from the draggedPieces list will cause them to be left behind where the
        // drag started.
        // NB. Pieces that have been dragged from a face-down Deck will be be Obscured to us, but
        // will be Obscured
        ...
        if (mergeWith instanceof Deck) {
          ...
          // If we rejected any dragged pieces for merging with the deck, update our list of dragged
          // pieces for this iteration of the loop
        ...
        // Add the remaining dragged counters to the target. If mergeWith is a single piece (not a
        // Stack or Deck), then
        // we are merging into an expanded Stack and the merge order must be reversed to maintain
        // the order of the merging
        // pieces.
      ...
      // Any piece we successfully moved, make sure is now considered a "selected piece" (i.e. a
      // member of KeyBuffer)
      for (final GamePiece piece : draggedPieces) {
        ...
        if (isMatSupport) {
          // If this is a piece that can be placed on mats, look for an overlapping mat, and put it
          // there.
          if (Boolean.TRUE.equals(piece.getProperty(MatCargo.IS_CARGO))) {
            ...
            if ((oldMat == null)
                || (!draggedPieces.contains(oldMat)
                    && !allDraggedPieces.contains(oldMat)
                    && !DragBuffer.getBuffer().contains(oldMat))) {
            ...
            if (thisMat != null) {
              ...
              for (final GamePiece cargo : contents) {
                ...
                if (draggedPieces.contains(cargo)
                    || allDraggedPieces.contains(cargo)
                    || DragBuffer.getBuffer().contains(cargo)) {
                  continue;
                }
    ...
    // We've finished the actual drag and drop of pieces, so we now create any auto-report message
    // that is appropriate.
    if (GlobalOptions.getInstance().autoReportEnabled()) {
      final Command report =
          createMovementReporter(comm)
              .getReportCommand()
    ...
    // Applies any apply-on-move keystroke (from the "Key command to apply to all units ending
    // movement on this map" field of the Map) to each piece
    ...
    return comm; // A command that, if executed, will fully replay the effects of this drag-and-drop
    // on another client, or via a logfile.
  }
   ...
   *
   * <p>On most systems, most of the drag-and-drop will be performed by the Drag Gesture Recognizer,
  ...
  /**
   * Checks if event has already been consumed -- so we will return false if this event has already
   * been handled by one of the fancier Drag Gesture Recognizers. Also screens out double-clicks and
   * anything with modifier keys down.
   *
   * @param e mouse event
  ...
  protected boolean canHandleEvent(MouseEvent e) {
    return !e.isConsumed()
        ...
        && e.getClickCount() == 1
        && // Previously also allowed a clickCount of zero here, causing
        // https://github.com/vassalengine/vassal/issues/12480
        (e.getButton() == MouseEvent.NOBUTTON || SwingUtils.isMainMouseButtonDown(e));
  }

  /**
   * @return true if this point is "close enough" to the point at which the user initially pressed
   *     the mouse button to be considered a mouse click (such that no drag-and-drop moves are
   *     processed)
   */
  public boolean isClick(Point pt) {
    ...
    if (dragBegin != null) {
      ...
      if (!breachedThreshold
          && map.mapToComponent(Math.abs(pt.x - dragBegin.x - curPieceOffsetX))
              <= GlobalOptions.getInstance().getDragThreshold()
          && map.mapToComponent(Math.abs(pt.y - dragBegin.y - curPieceOffsetY))
              <= GlobalOptions.getInstance().getDragThreshold()) {
        isClick = true;
      }
    }
  ...
  /**
   * Mouse button has been released -- if we can still handle the event (i.e. we haven't picked up
   * some exotic modifier key during the drag, etc), then we perform the drop.
   *
   * @param e Mouse Event -> NOTE that the supplied point is not the actual mouse position but
   *     rather it has been offset by how far off-center the drag point on the piece was/is.
   */
  @Override
  ...
  /**
   * Check if we have breached our drag threshold during a drag. Only meaningful in legacy DnD
   * situations.
   *
   * @param e mouse event.
   */
  ...
  public void mouseDragged(MouseEvent e) {
    if (!breachedThreshold && (dragBegin != null)) {
      ...
      if (map.mapToComponent(Math.abs(pt.x - dragBegin.x))
              > GlobalOptions.getInstance().getDragThreshold()
          || map.mapToComponent(Math.abs(pt.y - dragBegin.y))
              > GlobalOptions.getInstance().getDragThreshold()) {
  ...
  protected void performDrop(Point p) {
    ...
    GameModule.getGameModule()
        .sendAndLog(move); // Sends the command over the wire (and/or into the logfile)
    if (move != null) {
      DragBuffer.getBuffer()
          .clear(); // If we did anything, clear the drag buffer, as the DnD operation is complete.
      GameModule.getGameModule()
          .refreshVisibleMaps(); // Force any changed labels to redisplay on visible maps.
    }
  }
  ...
  /**
   * Implement Comparator to sort the contents of the drag buffer before completing the drag. This
   * sorts the contents to be in the same order as the pieces were in their original parent stack.
   */
  @Override
  ...
  public abstract static class AbstractDragHandler
    ...
    private static AbstractDragHandler theDragHandler =
        AbstractDragHandlerFactory.getCorrectDragHandler();
    ...
    public static class AbstractDragHandlerFactory {
      public static AbstractDragHandler getCorrectDragHandler() {
        if (!DragSource.isDragImageSupported()
            || GlobalOptions.getInstance().isForceNonNativeDrag()) {
    ...
    public static void resetDragHandler() {
      ...
      for (final Map map : Map.getMapList()) {
        ...
        map.getComponent()
            .setDropTarget(makeDropTarget(map.getComponent(), DnDConstants.ACTION_MOVE, map));
      }
     ...
     * DropTarget events are not multicast; there can be only one "true" listener.
     */
    public static DropTarget makeDropTarget(
        Component theComponent, int dndContants, DropTargetListener dropTargetListener) {
      if (dropTargetListener != null) {
        getTheDragHandler().dropTargetListeners.put(theComponent, dropTargetListener);
      }
    ...
    private int
        originalPieceOffsetX; // How far drag STARTED from GamePiece's center (on original map)
    private int originalPieceOffsetY;

    protected double dragPieceOffCenterZoom = 1.0; // zoom at start of drag

    protected int currentPieceOffsetX; // How far cursor is CURRENTLY off-center, a function of
     ...
     */
    @Deprecated(since = "2023-05-08", forRemoval = true)
    protected BufferedImage makeDragImageCursorCommon(
        double zoom, boolean doOffset, Component target, boolean setSize) {
      return makeDragImageCursorCommon(zoom, 1.0, doOffset, target);
    }

    protected BufferedImage makeDragImageCursorCommon(
        double mapzoom, double os_scale, boolean doOffset, Component target) {
      // FIXME: Should be an ImageOp for caching?
      final double zoom = mapzoom * os_scale;

      currentPieceOffsetX = (int) (originalPieceOffsetX / dragPieceOffCenterZoom * mapzoom + 0.5);
      currentPieceOffsetY = (int) (originalPieceOffsetY / dragPieceOffCenterZoom * mapzoom + 0.5);
    ...
    private void drawDragImage(
        BufferedImage image, Component target, List<Point> relativePositions, double zoom) {
      ...
      for (final PieceIterator dragContents = DragBuffer.getBuffer().getIterator();
        ...
        if (piece instanceof Stack) {
          ...
          piece.draw(
              g,
              EXTRA_BORDER - boundingBox.x + pos.x,
              EXTRA_BORDER - boundingBox.y + pos.y,
              map == null ? target : map.getView(),
              zoom);
        } else {
          ...
          final Highlighter highlighter =
              map == null ? BasicPiece.getHighlighter() : map.getHighlighter();
    ...
    /**
     * Sets things up at the beginning of a drag-and-drop operation: <br>
     * - Screen out any immovable pieces <br>
     * - Account for any offsets on in the window <br>
    ...
    protected Point dragGestureRecognizedPrep(DragGestureEvent dge) {
      ...
      final Map map =
          dge.getComponent() instanceof Map.View ? ((Map.View) dge.getComponent()).getMap() : null;

      final Point mousePosition =
          dge.getDragOrigin(); // BR// Bug13137 - now that we're not pre-adulterating dge's event,
      // it already arrives in component coordinates
      ...
      if (piecePosition.x <= 0 || piecePosition.y <= 0) {
        piecePosition = map == null ? mousePosition : map.componentToMap(mousePosition);
      }
      ...
      if (map != null) {
        ...
        if (parent != null) {
          final Point offset = parent.getStackMetrics().relativePosition(parent, piece);
    ...
    protected void beginDragging(DragGestureEvent dge) {
      ...
      final Point dragPointOffset =
          new Point(
              (int)
                  Math.round(
                      getOffsetMult()
                          * ((boundingBoxComp.x + currentPieceOffsetX) * os_scale - EXTRA_BORDER)),
              (int)
                  Math.round(
                      getOffsetMult()
                          * ((boundingBoxComp.y + currentPieceOffsetY) * os_scale - EXTRA_BORDER)));
   ...
   *
   * <p>Implementation of AbstractDragHandler when DragImage is supported by JRE. {@link
   * DragHandlerMacOSX} extends this for special Mac platform
   *
   * @author Pieter Geerkens
   */
  ...
  /**
   * Fallback drag-handler when DragImage not supported by JRE. Implements a pseudo-cursor that
   * follows the mouse cursor when user drags game pieces. Supports map zoom by resizing cursor when
   * it enters a drop target of type Map.View. <br>
   *
   * @author Jim Urbas
   * @version 0.4.2
   */
  public static class DragHandlerNoImage extends AbstractDragHandler {
    ...
    protected final Point drawOffset =
        new Point(); // translates event coords to local drawing coords
    ...
    /**
     * Installs the cursor image into our dragCursor JLabel. Sets current zoom. Should be called at
     * beginning of drag and whenever zoom changes. INPUT: DragBuffer.getBuffer OUTPUT:
    ...
    public void dragEnter(DropTargetDragEvent e) {
      ...
      if (newDropWin != dropWin) {
        final double newZoom =
            newDropWin instanceof Map.View ? ((Map.View) newDropWin).getMap().getZoom() : 1.0;
