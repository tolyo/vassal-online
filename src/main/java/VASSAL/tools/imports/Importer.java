...
public abstract class Importer {
  ...
  protected static void insertComponent(Buildable child, Buildable parent) {
    child.build(null);
    if (child instanceof PieceSlot) ((PieceSlot) child).updateGpId(GameModule.getGameModule());
    child.addTo(parent);
    parent.add(child);
  }
  ...
  protected LayeredPieceCollection getLayeredPieceCollection() {
    ...
    final List<LayeredPieceCollection> l = map.getComponentsOf(LayeredPieceCollection.class);
  ...
  /**
   * The method that actually loads the file and creates the classes containing information needed
   * for the archive. The archive is written in <code>writeToArchive</code>.
   *
  ...
  /**
   * Create the VASSAL module based on the classes created by <code>load</code>. This should not be
   * called directly but rather <code>importFile</code>.
   *
   * @throws IOException oops
   */
  ...
  /**
   * Two methods are needed to import a file. <code>Importer.importFile</code> initializes the game
   * module and calls <code>load</code> which must be overridden by descendants.
   *
   * @param action <code>ImportAction</code> which creates the <code>Importer</code>. This is needed
   *     for file dialogs that may be called by <code>Importer</code> methods.
   * @param f The base file to be imported.
   * @throws IOException oops
   */
  ...
  public static String stripExtension(String s) {
    ...
    if (index == -1 || index < pathIdx) return s;
    else return s.substring(0, index);
  }

  /**
   * Read a null-terminated string representing a Windows file name and convert Windows separator
   * characters <tt>'\\'</tt> to the local separator character. This is the default file name format
   * for many imported modules and should be used whenever a filename is read as a null-terminated
   * string in order to ensure platform independence.
   */
  public static String readWindowsFileName(InputStream in) throws IOException {
    ...
    do {
      ch = (char) in.read();
      if (ch == '\\') sb.append(File.separatorChar);
  ...
  /**
   * Read a null-terminated string from a file up to a maximum length which includes the null
   * termination. If the actual string is longer, no more bytes will be read.
   */
  public static String readNullTerminatedString(InputStream in, int maxLen) throws IOException {
    final StringBuilder sb;
    if (maxLen == 0) sb = new StringBuilder();
    else sb = new StringBuilder(maxLen);
    char ch;
    for (int i = 0; maxLen == 0 || i < maxLen; ++i) {
      ch = (char) in.read();
      if (ch >= 0x20 && ch <= 0x7e) sb.append(ch);
      else if (ch != 0) sb.append(' ');
      else break;
    }
    return sb.toString();
  }
  ...
  /**
   * Get a unique file name for an image in the game module archive. This function tests the
   * provided name against those that are already present in the archive and if the file name
   * already exists, creates an alternate, unique file name. If an alternate is created, it is of
   * the form <code><tt>name</tt> + "(<tt>n</tt>)"</code> where <tt>n</tt> is an integer.
   */
  public static String getUniqueImageFileName(String s, String ext) {
    ...
    try {
      while (writer.contains(writer.getImagePrefix() + t + ext)) t = s + '(' + (++index) + ')';
    } catch (IOException e) {
      // FIXME: ????
    }
  ...
  public static String getExtension(String s) {
    ...
    final int pathIdx = s.lastIndexOf(File.separatorChar);
    if (extIdx == -1 || extIdx < pathIdx || extIdx >= s.length() - 1) return "";
    else return s.substring(extIdx + 1);
  }
