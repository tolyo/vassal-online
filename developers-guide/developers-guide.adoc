...
* Maven plugin configuration in link:{rootdir}pom.xml[]
* Checkstyle configuration in link:{rootdir}src/test/resources/checkstyle-checks.xml[]
* Exceptions from checks in link:{rootdir}config/checkstyle/suppressions.xml[]
...
* Maven plugin version in link:{rootdir}pom.xml[]
* Maven plugin configuration in link:{rootdir}pom.xml[]
* Spotbugs include filter configuration in link:{rootdir}src/test/resources/spotbugs-include-filter.xml[]
...
* Maven plugin version in link:{rootdir}pom.xml[]
* Maven plugin configuration in link:{rootdir}pom.xml[]
* Configuration of ignores in link:{rootdir}src/test/resources/clirr-ignored-differences.xml[]

===== Documentation References

The AsciiDoc sources under link:{rootdir}vassal-doc/[] are kept in the repository as reference material, but they are no longer part of the Maven reactor or release build.
...
====== Examples
* Compile only, build jar in `target`, skip tests, checkstyle, spotbugs, javadoc, clirr:
[source]
----
mvn clean package -DskipTests=true -Dcheckstyle.skip=true -Dspotbugs.skip=true -Dmaven.javadoc.skip=true -Dclirr.skip=true
----
...
** Our forum: https://forum.vassalengine.org/c/news/17 (Pin the new post, unpin the one for the previousth release.)
...
* [ ] Update the `sources` section in `org.vassalengine.vassal/org.vassalengine.vassal.yml` in the https://github.com/flathub/org.vassalengine.vassal repository.
* [ ] Relax.
