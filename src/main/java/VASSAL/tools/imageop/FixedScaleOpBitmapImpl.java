...
public class FixedScaleOpBitmapImpl extends AbstractTiledOpImpl implements ScaleOp {
  ...
  public FixedScaleOpBitmapImpl(
    ...
    if (width <= 0) throw new IllegalArgumentException("Cannot scale image to width " + width);
    if (height <= 0) throw new IllegalArgumentException("Cannot scale image to height " + height);
    ...
    hash = new HashCodeBuilder().append(sop).append(scale).append(size).append(hints).toHashCode();
  }
  ...
  public boolean equals(Object o) {
    ...
    return size.equals(op.size) && sop.equals(op.sop) && hints.equals(op.hints);
  }
