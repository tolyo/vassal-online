...
public class SpecialDie extends AbstractConfigurable {
  ...
  private final FormattedString format =
      new FormattedString("$" + RESULT + "$"); // $NON-NLS-1$ //$NON-NLS-2$
  ...
  public String getTextValue(int face) {
    ...
    // No Faces may be defined, or opponent may have a version of the module with more faces defined
    // than we have
    ...
    if (face < 0 || face >= faceCount) {
      ...
      format.setProperty(RESULT, aFace.getLocalizedConfigureName());
