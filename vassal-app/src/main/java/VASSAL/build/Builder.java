...
public class Builder {
  ...
  /**
   * General building algorithm. For each sub-element of the build Element, this method creates an
   * instance of the class (which must implement Buildable) whose name matches the XML element tag
   * name, builds that instance with the sub-element, and adds it to the parent Buildable
   *
   * <p>This algorithm calls a component's {@link Buildable#build} method before calling its {@link
   * Buildable#addTo} method
   *
      ...
      if (Node.ELEMENT_NODE == child.getNodeType()) {
        try {
          ...
          logger.error(
              "Error building "
                  + child.getNodeName()
                  + (attr != null ? " " + attr.getNodeValue() : "")); // NON-NLS
          throw ex;
        }
      }
    }
   ...
   *
   * <p>The <code>.class</code> file for the named class may be either in the System's classpath or
   * else within the {@link DataArchive} of the {@link GameModule}.
   *
   * @throws IllegalBuildException if something goes wrong when loading the class or creating an
   *     instance of it
   */
  public static Buildable create(Element e, Buildable parent) throws IllegalBuildException {
    ...
    try {
      final Buildable b =
          (Buildable)
              (mod == null ? Class.forName(name) : mod.getDataArchive().loadClass(name))
                  .getConstructor()
                  .newInstance();

      // BR// We do an early & extra set of the ancestor during build so that if, during the addTo()
      // sequence,
      // BR// an item in an AbstractFolder needs to know its "first non-folder ancestor", it can
      // walk up the
      // BR// tree as necessary.
      // BE// NOTE: Since we haven't actually checked if the child is an allowed component of the
      // parent, it's
      // BE// possible the parent may be a completely invalid ancestor. This can cause an
