...
public class Server extends Thread {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String reportURL =
        p.getProperty("URL", "https://vassalengine.org/util/"); // $NON-NLS-1$ //$NON-NLS-2$
    ...
    if (p.getProperty("test") != null) { // $NON-NLS-1$
      ...
      handler.writeLine(
          Protocol.encodeRegisterCommand(
              "rk", "test/Main Room", "")); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      ...
      try {
        ...
        while ((line = reader.readLine()) != null) {
          if (line.startsWith("JOIN")) { // $NON-NLS-1$
            ...
            final String msg =
                Protocol.encodeForwardCommand("test/*", buffer.toString()); // $NON-NLS-1$
