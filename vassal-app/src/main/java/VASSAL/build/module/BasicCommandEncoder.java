 ...
 * {@link RemovePiece}, {@link ChangePiece}, {@link MovePiece}, this class is most commonly needed
 * by module designers who want to make custom "Traits" for game pieces because it contains {@link
 * #createDecorator}, the {@link DecoratorFactory} for Traits, which are usually internally referred
 * to as Decorators because they are implemented using the <a
 * href="https://en.wikipedia.org/wiki/Decorator_pattern">Decorator Pattern</a>. If a module is to
 * add its own custom game pieces, it will need to override the {@link #createDecorator} method, and
 ...
 * </pre>
 *
 * Then in the buildFile (XML) for the module, the VASSAL.build.module.BasicCommandEncoder entry is
 * replaced with an entry for (in the example above): <code>MyCustomClasses.MyCustomCommandEncoder
 * </code> The class files are placed in a <code>MyCustomClasses</code> (package name) folder in the
 * Zip structure of the VMOD file, and At that point the new trait can be imported into a piece as
 * <code>MyCustomClasses.MyCustomClass</code> (package name followed by class name).
 *
 * <p>Less often, the {@link #createBasic} or {@link #createPiece} methods could be overridden to
 * allow instantiation of custom {@link GamePiece} classes.
 */
public class BasicCommandEncoder implements CommandEncoder, Buildable {
  /**
   * Factory interface for Decorators
   *
   * <p>See: <a href="https://en.wikipedia.org/wiki/Decorator_pattern">Decorator Pattern</a>, <a
   * href="https://en.wikipedia.org/wiki/Factory_method_pattern">Factory Pattern</a>
   */
  @FunctionalInterface
  ...
  /**
   * Factory interface for BasicPieces. See: <a
   * href="https://en.wikipedia.org/wiki/Factory_method_pattern">Factory Pattern</a>
   */
  @FunctionalInterface
  ...
  /** Factories for the three Basic Piece types that can be created: Stack, BasicPiece, and Deck. */
  private final Map<String, BasicPieceFactory> basicFactories =
      Map.ofEntries(
  ...
  /**
   * Factories for all of the standard Trait (aka "Decorator") types. If a new standard Trait is to
   * be added to *VASSAL* it must be added here (as opposed to adding a Custom Class trait, which is
   * added per the documentation in {@link BasicCommandEncoder}'s main Javadoc entry).
   */
  ...
  private final DecoratorFactory defaultDecoratorFactory =
      (type, inner) -> {
  ...
  /**
   * Creates a {@link Decorator} instance - a {@link GamePiece} "Trait". Modules which wish to
   * provide their own custom classes should subclass BasicCommandEncoder and override this class.
   * The override should check for and parse any definitions that match desired custom
   * Traits(Decorators), and then use super to call this method for any unmatched definitions.
   * Further documentation on creating custom traits appears in BasicCommandEncoder's own javadoc
   * entry, or alternatively at the top of BasicCommandEncoder.java.
   *
   * @param type the type of the Decorator ("Trait") to be created. Once created, the Decorator
   *     should return this value from its {@link Decorator#myGetType} method.
   * @param inner the inner trait/piece of the Decorator (the "innermost" member of a game piece
   *     will be a {@link BasicPiece}; each successive Trait in the trait list presented in a
   *     piece's {@link PieceDefiner} dialog represents a sep "outward").
   * @see Decorator, <a href="https://en.wikipedia.org/wiki/Decorator_pattern">Decorator Pattern</a>
   */
  public Decorator createDecorator(String type, GamePiece inner) {
    return decoratorFactories
        .getOrDefault(typePrefix(type), defaultDecoratorFactory)
        .createDecorator(type, inner);
  }

  /**
   * Create a {@link GamePiece} instance that is not a Decorator ("Trait"). In other words a {@link
   * BasicPiece}, a {@link Stack}, or a {@link Deck}.
   *
   * @param type the type of the GamePiece. The created piece should return this value from its
   *     {@link GamePiece#getType} method.
   */
  protected GamePiece createBasic(String type) {
    return basicFactories
        .getOrDefault(typePrefix(type), defaultBasicPieceFactory)
        .createBasicPiece(type);
  }

  /**
   * Creates a GamePiece instance from the given type information. Determines from the type whether
   * the represented piece is a {@link Decorator} ("Trait") or not and forwards to {@link
   * #createDecorator} or {@link #createBasic}. This method should generally not need to be
   * overridden. Instead, override {@link #createDecorator} or {@link #createBasic}
   *
  ...
  public GamePiece createPiece(String type) {
    ...
    if (innerType != null) {
      ...
      if (inner == null) {
        ErrorDialog.dataWarning(
            new BadDataReport("Could not create piece with type " + innerType, type)); // NON-NLS
  ...
  /**
   * Adds a buildable subcomponent. BasicCommandEncoder doesn't normally have subcomponents, so this
   * is empty.
   *
  ...
  /**
   * Deserializes a string into a Basic Piece command (Add, Remove, Change, Move, and... Play Audio
   * Clip!), readying it for execution.
   *
  ...
  /**
   * Serializes a Basic Piece command (Add, Remove, Change, Move, and ... Play Audio Clip!) into a
   * String, readying it for transmission to other clients.
   *
