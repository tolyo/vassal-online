...
/**
 * Allows a single trait to generate multiple context menu commands, using a set of filtered map
 * locations. The same key command is generated by any of these context menu commands, which can
 * then be distinguished using the provided properties.
 */
public class MultiLocationCommand extends Decorator implements TranslatablePiece {
  ...
  public static final String[] LOC_KEYS = {
    "Editor.MultiLocationCommand.regions", "Editor.MultiLocationCommand.zones"
  };
  ...
  public static class MultiLocationKeyCommand extends KeyCommand {
    ...
    public MultiLocationKeyCommand(
        String name,
        NamedKeyStroke key,
        GamePiece target,
    ...
    public void actionPerformed(ActionEvent evt) {
      ...
      gm.setLocationKeyCommand(
          this); // So we can figure out what the hell we're talking about during myKeyEvent later
      // on
  ...
  /**
   * Makes our special location-currently-being-evaluated information available to our
   * property-match evaluation; other than that, properties from the piece as usual
   */
  ...
  public void mySetType(String type) {
    ...
    menuText.setFormat(
        st.nextToken(Resources.getString("Editor.MultiLocationCommand.loc_default_command")));
  ...
  /**
   * Try out an individual location (region or zone depending on our locType) -- if it passes our
   * Properties Expression, then make some menu text for it (using our report format) and add it to
   * our key commands list.
   *
    ...
    if (propertiesFilter.getExpression().isBlank() || propertiesFilter.isTrue(locPS)) {
      final AuditTrail audit =
          AuditTrail.create(
              locPS,
              menuText.getFormat(),
              Resources.getString("Editor.MultiLocationCommand.evaluate_menu_text"));
      final String myMenuText = menuText.getLocalizedText(locPS, this, audit);
      keyCommands.add(
          new MultiLocationKeyCommand(
              myMenuText,
              key,
              getOutermost(this),
              this,
  ...
  /**
   * Here is when we're being asked, essentially for "what goes on our context menu". So this is
   * when we search through maps, boards, grids, zones and regions to generate our items
   *
  ...
  /**
   * If the stroke matches our key command, then call the funky "like a static but not actually
   * static" GameModule thing to retrieve the specific menu command being processed -- that is then
   * used to set values for our special trait properties (e.g. LocationOfCommand, etc) in the
   * scratchpad of our piece (so now other traits receiving this key command can read them)
   *
  ...
  public static class Ed implements PieceEditor {
    ...
    public Ed(MultiLocationCommand p) {
      ...
      propertyMatchInput.setContextLevel(
          Configurer.ContextLevel.MAP); // This PME is NOT applied to pieces
