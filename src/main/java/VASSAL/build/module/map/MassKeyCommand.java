...
/**
 * Adds a button to a map window toolbar. Hitting the button applies a particular key command to all
 * pieces on that map with a given name.
 *
...
public class MassKeyCommand extends AbstractToolbarItem implements RecursionLimiter.Loopable {
  ...
  /**
   * @return Our GKC type -- this method is overridden by {@link
   *     VASSAL.build.module.GlobalKeyCommand} for module-level GKC's and by {@link
   *     DeckGlobalKeyCommand} for Deck GKC's. This value affects what configurer options are shown.
   */
  ...
  public MassKeyCommand() {
    ...
    setLaunchButton(
        makeLaunchButton(
            ...
            Resources.getString("Editor.GlobalKeyCommand.button_name"),
            "", // Default art exists, but is a little weird, and wasn't actually being defaulted to
            // before --> "/images/keyCommand.gif", //NON-NLS
            e -> apply()));
  ...
  /**
   * Copy an existing MassKeyCommand or subclass getAttributeNames will resolve to the subclass, so
   * only attributes unique to the subclass will be copied CONDITION is a legacy field that must be
   * null, not "" when not used
   */
  ...
  public void apply() {
    ...
    final AuditTrail audit =
        AuditTrail.create(this, "", Resources.getString("Editor.MassKey.match"));
    if (singleMap) {
      GameModule.getGameModule()
          .sendAndLog(globalCommand.apply(map, getFilter(audit), target, audit));
    } else {
      final List<Map> l = Map.getMapList();
      GameModule.getGameModule()
          .sendAndLog(globalCommand.apply(l.toArray(new Map[0]), getFilter(audit), target, audit));
    }
  }
  ...
  public String[] getAttributeDescriptions() {
    if (condition == null) {
      return ArrayUtils.addAll(
          ...
          Resources.getString(
              "Editor.GlobalKeyCommand.global_key_command"), //$NON-NLS-1$       // Key Command
          Resources.getString(
              "Editor.MassKey.counters"), //$NON-NLS-1$       // Apply to counters on this map only
          ...
          Resources.getString(
              "Editor.MassKey.suppress"), //$NON-NLS-1$       // Suppress individual reports?
      ...
      return ArrayUtils.addAll(
          ...
          Resources.getString("Editor.MassKey.match"), // $NON-NLS-1$           // Match properties
          Resources.getString(
              "Editor.MassKey.deck_content"), //$NON-NLS-1$    // Apply to pieces in deck
          Resources.getString(
              "Editor.MassKey.suppress"), //$NON-NLS-1$       // Suppress individual reports?
          Resources.getString("Editor.report_format"), // $NON-NLS-1$          // Report format
  ...
  public static class DeckPolicyConfig extends Configurer implements ConfigurerFactory, Auditable {
    ...
    /**
     * Legacy version for binary compability. Will attempt to return something resembling the old
     * integer value.
     *
    ...
    public int getIntValue() {
      ...
      try {
        ...
        return Integer.parseInt(
            exp.tryEvaluate(GameModule.getGameModule(), this, "Editor.GlobalKeyCommand.zone_name"));
      }
    }
          ...
          new BooleanAndPieceFilter(
              ...
              piece -> {
                ...
                if (ALWAYS.equals(condition)) {
                  ...
                  valid =
                      Embellishment.getLayerWithMatchingActivateCommand(piece, stroke, true)
                          != null;
                } else if (IF_INACTIVE.equals(condition)) {
                  valid =
                      Embellishment.getLayerWithMatchingActivateCommand(piece, stroke, false)
                          != null;
                }
                return valid;
              });
