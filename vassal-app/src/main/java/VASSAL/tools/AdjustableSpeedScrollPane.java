...
/**
 * AdjustableSpeedScrollPane extends {@link ScrollPane} by making the scroll speed
 * user-configurable. Use AdjustableScrollPane instead of ScrollPane wherever a scrollpane for large
 * images is needed.
 *
...
public class AdjustableSpeedScrollPane extends ScrollPane {
  ...
  /**
   * Creates an AdjustableSpeedScrollPane that displays the contents of the specified component,
  ...
  /**
   * Creates an AdjustableSpeedScrollPane that displays the view component in a viewport with the
   * specified scrollbar policies. The available policy settings are listed at {@link
  ...
  public AdjustableSpeedScrollPane(Component view, int vsbPolicy, int hsbPolicy) {
    ...
    config.addPropertyChangeListener(
        e -> {
          if (SCROLL_SPEED.equals(e.getPropertyName())) setSpeed((Integer) e.getNewValue());
        });
