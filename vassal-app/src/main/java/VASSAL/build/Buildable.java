...
/**
 * A Buildable is an object which can be initialized from an XML configuration file (e.g. <code>
 * "buildFile"</code>) in which each XML element represents an component object. The Buildable
 * objects are built into a component hierarchy that mirrors the XML structure. Many Buildables in a
 * vassal module are also {@link Configurable}, so that their attributes can be configured in the
 * Editor. SEE ALSO: {@link AbstractBuildable}, {@link AbstractConfigurable}
 */
public interface Buildable {
  ...
  /**
   * Adds this component to its parent. In order to make Buildable objects extensible, the child is
   * responsible for adding itself to the parent. That way, Buildable subcomponents can be defined
  ...
  /**
   * Adds a child component. Both this method and {@link #addTo} are invoked when adding a child to
   * a parent
   *
