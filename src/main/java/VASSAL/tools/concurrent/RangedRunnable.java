 ...
 *
 * <p>A {@code RangedRunnable} can be updated after it has been submitted but before it has been
 * run. This cuts down on the number of times the {@code RangedRunnable} must be submitted, and is
 * appropriate for values where only the most recent update matters.
 *
...
public abstract class RangedRunnable<T> implements Runnable {
   ...
   * {@inheritDoc}
   *
   * <p>Flushes the range and calls {@see #run(Pair<T,T>)}.
   */
  @Override
   ...
   *
   * <p>This method may be executed only from {@see #setLast}.
   */
  ...
  /**
   * Returns the old range and creates a new range adjacent to the old one. The upper bound of the
   * old range becomes the lower bound of the new range.
   *
