...
/**
 * Provides support for hidden panels. Use the split methods to create an instance of {@link
 * SplitPane}, which can then be manipulated to show and hide the panel
 *
...
public class ComponentSplitter {
   ...
   * @param resizeOnVisibilityChange If true, the containing window will expand or shrink to an
   *     appropriate size when the hideable component is shown or hidden
   * @return the {@link SplitPane} containing the two components
   */
  public SplitPane splitRight(
   ...
   * @return the {@link SplitPane} containing the two components
   */
  public SplitPane splitBottom(
      Component base, Component hideableComponent, boolean resizeOnVisibilityChange) {
    return split(base, hideableComponent, SplitPane.HIDE_BOTTOM, resizeOnVisibilityChange);
  }

  /**
   * Create a new hideable panel to the top of the base component. The base component is replaced by
   * a {@link SplitPane}
   *
   * @param base the base component
   * @param hideableComponent the hideable component
   ...
   * @return the {@link SplitPane} containing the two components
   */
  public SplitPane splitTop(
      Component base, Component hideableComponent, boolean resizeOnVisibilityChange) {
    return split(base, hideableComponent, SplitPane.HIDE_TOP, resizeOnVisibilityChange);
  }

  /**
   * Search the containment hierarchy for the index-th {@link SplitPane} ancestor of a target
   * component
   *
  ...
  /**
   * Search the containment hierarchy for the ith {@link SplitPane} ancestor of a target component
   *
   * @param c the target component
   * @param index If -1, return the last {@link SplitPane} ancestor
   * @return the {@link SplitPane} ancestor, or the original component if none is found
  ...
  /**
   * Create a new hideable panel beside the base component. The base component is replaced by a
   * {@link SplitPane}
   *
   * @param base the base component
   * @param newComponent the hideable component, one of {@link SplitPane#HIDE_TOP}, {@link
   *     SplitPane#HIDE_RIGHT}, {@link SplitPane#HIDE_BOTTOM}, {@link SplitPane#HIDE_LEFT}
  ...
  /**
   * Contains methods to automatically show/hide one of its components (the "hideable" component)
   * while the other (the "base" component) remains always visible. Can optionally change the size
   * of its top level ancestorExtension when the component is shown/hidden. The hideable component
   * is initially hidden
   */
  public static class SplitPane extends JSplitPane {
     ...
     * @param hideablePosition one of {@link #HIDE_TOP}, {@link #HIDE_BOTTOM}, {@link #HIDE_LEFT} or
     *     {@link #HIDE_RIGHT}
     * @param resizeOnVisibilityChange If true, resize the top-level ancestor when the hideable
     *     component is shown/hidden
     */
    public SplitPane(
        Component hideableComponent,
        Component baseComponent,
        int hideablePosition,
        boolean resizeOnVisibilityChange) {
      super(
          HIDE_TOP == hideablePosition || HIDE_BOTTOM == hideablePosition
              ? VERTICAL_SPLIT
              : HORIZONTAL_SPLIT);
          ...
          if (ancestor != null) {
            switch (hideablePosition) {
              ...
              case HIDE_RIGHT:
                ancestor.setSize(
                    new Dimension(
                        ancestor.getSize().width - getHideableComponent().getSize().width,
                        ancestor.getSize().height - getDividerSize()));
                break;
              case HIDE_TOP:
              case HIDE_BOTTOM:
                ancestor.setSize(
                    new Dimension(
                        ancestor.getSize().width,
                        ancestor.getSize().height
                            - getHideableComponent().getSize().height
                            - getDividerSize()));
                break;
            }
            ancestor.validate();
    ...
    /**
     * Set the divider location and/or the top-level ancestor size to be large enough to display the
     * argument {@link SplitPane}'s hideable component
     *
    ...
    /**
     * Set the base component size to be large enough to accommodate all descendant SplitPane's
     * showing components
     */
    ...
    /**
     * Return the preferred size of the top-level container in the direction transverse to this
     * SplitPane's orientation. Depends on which ancestors have been shown using {@link
      ...
      if (resizeOnVisibilityChange) {
        ...
        switch (getOrientation()) {
          ...
          case VERTICAL_SPLIT:
            ...
            h =
                Math.min(
                    ancestorSize.height + prefHSize.height, screenBounds.height - ancestorPos.y);
        ...
        case HIDE_BOTTOM:
          loc =
              getSize().height
                  - getInsets().bottom
                  - getDividerSize()
                  - getRightComponent().getPreferredSize().height;
      }
      return loc;
    }
  ...
  public static void main(String[] args) {
    ...
    final SplitPane splitBottom =
        split(splitAncestorOf(main, -1), new ScrollPane(large), SplitPane.HIDE_BOTTOM, true);
    splitBottom.setResizeWeight(0.0);
    main.add(text, BorderLayout.CENTER);
    toolbar.add(
        new AbstractAction("Left") { // NON-NLS
          private static final long serialVersionUID = 1L;
    ...
    toolbar.add(
        new AbstractAction("Right") { // NON-NLS
          private static final long serialVersionUID = 1L;
    ...
    toolbar.add(
        new AbstractAction("Bottom") { // NON-NLS
          private static final long serialVersionUID = 1L;
