...
public class Symbol {
  ...
  public void draw(Graphics g, Rectangle bounds, Color fg, Color bg, Color sz, float lineWidth) {
    if (symbolSetName.equals(NATO)) {
      NatoUnitSymbolSet.draw(
          symbolName1, symbolName2, g, bounds, fg, bg, sz, lineWidth, symbolSize);
    }
  }
  ...
  public static class NatoUnitSymbolSet {
    ...
    public static final SizeOption[] SIZES = {
      ...
      new SizeOption(
          SZ_INSTALLATION,
          Resources.getString("Editor.Symbol.Size.installation"),
          1,
          INSTALLATION_SYMBOL),
      ...
      // Note: Platoon and Section constants reversed due to earlier error. Maintain this switch for
      // compatibility
      new SizeOption(
          SZ_PLATOON, Resources.getString("Editor.Symbol.Size.section"), 2, SQUAD_SYMBOL),
      new SizeOption(
          SZ_SECTION, Resources.getString("Editor.Symbol.Size.platoon"), 3, SQUAD_SYMBOL),
      new SizeOption(
          SZ_ECHELON, Resources.getString("Editor.Symbol.Size.echelon"), 4, SQUAD_SYMBOL),
      new SizeOption(
          SZ_COMPANY, Resources.getString("Editor.Symbol.Size.company"), 1, COMPANY_SYMBOL),
      new SizeOption(
          SZ_BATTALION, Resources.getString("Editor.Symbol.Size.battalion"), 2, COMPANY_SYMBOL),
      new SizeOption(
          SZ_REGIMENT, Resources.getString("Editor.Symbol.Size.regiment"), 3, COMPANY_SYMBOL),
      new SizeOption(
          SZ_BRIGADE, Resources.getString("Editor.Symbol.Size.brigade"), 1, BRIGADE_SYMBOL),
      new SizeOption(
          SZ_DIVISION, Resources.getString("Editor.Symbol.Size.division"), 2, BRIGADE_SYMBOL),
      ...
      new SizeOption(
          SZ_ARMY_GROUP, Resources.getString("Editor.Symbol.Size.army_group"), 5, BRIGADE_SYMBOL),
    ...
    protected static void draw(
        Graphics g, float lineWidth, String name, Rectangle bounds, boolean drawLow) {
      ...
      switch (name) {
        case AIRBORNE:
          {
            ...
            g2.draw(
                new Arc2D.Double(x1, y1, bounds.width / 4, bounds.height / 4, 0, 180, Arc2D.OPEN));
            g2.draw(
                new Arc2D.Double(
                    x_center, y1, bounds.width / 4, bounds.height / 4, 0, 180, Arc2D.OPEN));
        ...
        case AIR_FORCE:
          {
            ...
            g2.draw(
                new Arc2D.Double(
                    x_center - xoff2 - xoff1,
                    y_top + yoff,
                    xoff1 * 2,
                    bounds.height - (2 * yoff),
                    90,
                    180,
                    Arc2D.OPEN));
            g2.draw(
        ...
        case ARMORED:
          {
            ...
            g.drawLine(
                x_left + xoff1 + xoff2, y_bottom - yoff, x_right - xoff1 - xoff2, y_bottom - yoff);
            g2.draw(
                new Arc2D.Double(
                    x_left + xoff1,
                    y_top + yoff,
                    xoff2 * 2,
                    bounds.height - (yoff * 2),
                    90,
                    180,
                    Arc2D.OPEN));
            g2.draw(
                new Arc2D.Double(
                    x_right - xoff1 - (2 * xoff2),
                    y_top + yoff,
                    xoff2 * 2,
                    bounds.height - (yoff * 2),
                    270,
        ...
        case ENGINEERS:
          {
            ...
            final BasicStroke estroke =
                new BasicStroke(
                    oldStroke.getLineWidth() * 1.2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
        ...
        case GLIDER:
          g.drawLine(
              x_left + (x_center - x_left) / 3,
              y_center,
              x_right - (x_center - x_left) / 3,
              y_center);
          break;
        case GUERILLA:
          {
        ...
        case NAVY:
          {
            ...
            g.drawLine(
                x_center - xoff1, y_top + yoff1 + yoff2, x_center + xoff1, y_top + yoff1 + yoff2);
            g2.draw(
                new Arc2D.Double(
                    x_center - xoff2,
                    y_top + yoff1,
                    xoff2 * 2,
                    bounds.height - (2 * yoff1),
                    225,
                    90,
                    Arc2D.OPEN));
            break;
    ...
    public static BufferedImage buildSizeImage(
        Graphics g, int count, String type, int sym_w, int sym_h, int gap) {
