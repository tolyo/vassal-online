...
public class RefreshPredefinedSetupsDialog extends JDialog {
  ...
  private void initComponents() {
    optionsUserMode =
        true; // protects against change conflicts whilst vassal controls panel settings that are
    // subject to stateChanged event handling
    ...
    // refreshButton.setEnabled(true); // this may be belt & braces - re-enabled anyway on a
    // cancelled refresh (otherwise whole window is closed)
    ...
    refreshPieces.setEnabled(
        false); // this is the standard default - locked as part of ensuring that at least one main
    // option is on
    ...
    deletePieceNoMap =
        new JCheckBox(Resources.getString("GameRefresher.delete_piece_no_map"), true);
    ...
    reportOff =
        new JCheckBox(Resources.getString("Editor.RefreshPredefinedSetups.reportOff"), false);
    ...
    final JPanel filterPanel =
        new JPanel(
            new MigLayout(
                ConfigurerLayout.STANDARD_INSETS_GAPY, "[]rel[grow,fill,push]")); // NON-NLS
    filterPanel.add(
        new JLabel(Resources.getString("Editor.RefreshPredefinedSetups.filter_prompt")), "");
    pdsFilterBox =
        new HintTextField(32, Resources.getString("Editor.RefreshPredefinedSetups.filter_hint"));
  ...
  private void refreshPredefinedSetups() {
    ...
    if (isFilterMode()) {
      ...
      try {
        // matching, assuming Regex with no escape of the end string modifier, otherwise match all
        // to end
      ...
      pdsFilter =
          pdsFilter
              .toLowerCase(); // original search string will be used for case-insensitive string
      // search
    }
    ...
    final List<PredefinedSetup> modulePdsAndMenus =
        mod.getAllDescendantComponentsOf(PredefinedSetup.class);
    ...
    for (final PredefinedSetup pds : modulePdsAndMenus) {
      if (!pds.isMenu() && pds.isUseFile()) {
        ...
        if (pdsFile != null
            ...
            && (pdsFilter == null
                || (pdsName != null
                        && (pdsName.toLowerCase().contains(pdsFilter)
                            || (filterPattern != null
                                && (filterPattern.matcher(pdsName).matches()
                                    || filterPattern2.matcher(pdsName).matches())))
                    || (pdsFile.toLowerCase().contains(pdsFilter)
                        || (filterPattern != null
                            && (filterPattern.matcher(pdsFile).matches()
                                || filterPattern2.matcher(pdsFile).matches())))))) {
    ...
    if (pdsCount > 0) {
      ...
      log(
          "|<b>"
              + Resources.getString(
                  "Editor.RefreshPredefinedSetupsDialog.start_refresh",
                  mod.getGameVersion(),
                  isRefreshOfExtension
                      ? " " + Resources.getString("Editor.RefreshPredefinedSetupsDialog.extension")
                      : ""));
      ...
      if (isTestMode())
        log(
            GameRefresher.ERROR_MESSAGE_PREFIX
                + Resources.getString("GameRefresher.refresh_counters_test_mode"));
      if (isFilterMode())
        log(
            GameRefresher.ERROR_MESSAGE_PREFIX
                + Resources.getString(
                    "Editor.RefreshPredefinedSetups.setups_filter",
                    ConfigureTree.noHTML(pdsFilter)));
      ...
      for (final PredefinedSetup pds : modulePds) {
        ...
        this.setTitle(
            Resources.getString("Editor.RefreshPredefinedSetupsDialog.progress", ++i, pdsCount, pct)
                + (warningPds.isEmpty()
                    ? ""
                    : "  "
                        + Resources.getString(
                            "Editor.RefreshPredefinedSetupsDialog.errors",
                            lastErrorFile,
                            warningPds.size() - 1)));
        this.setCursor(waitCursor);
        ...
        if (i > 1 && pdsFileProcessed(modulePds.subList(0, i - 1), pdsFile)) {
          ...
          if (!options.contains(GameRefresher.SUPPRESS_INFO_REPORTS)) {
            ...
            log(
                Resources.getString(
                    Resources.getString(
                        "Editor.RefreshPredefinedSetupsDialog.skip",
                        pds.getAttributeValueString(PredefinedSetup.NAME),
                        pdsFile)));
          }
        } else {
          gs.setup(false); // BR// Ensure we clear any existing game data/listeners/objects out.
          mod.setRefreshingSemaphore(
              true); // BR// Raise the semaphore that suppresses GameState.setup()

          try {
            // FIXME: At this point the Refresh Options window is not responsive to Cancel, which
            // means that runs can only be interrupted by killing the Vassal editor process
            ...
            // Ignore the Exception, it has already been reported into the refresher log by
            // pds.refreshWithStatus().
            failPds.add(pds);
          } finally {
      ...
      if (!isTestMode()) {
        if (alertOn.isSelected()) { // sound alert
          final SoundConfigurer c =
              (SoundConfigurer) Prefs.getGlobalPrefs().getOption("wakeUpSound");
          c.play();
        }
      ...
      gs.setup(
          false); // BR// Clear out whatever data (pieces, listeners, etc.) left over from final
      // game loaded.
      ...
      if (!warningPds.isEmpty()) {
        log(
            GameRefresher.ERROR_MESSAGE_PREFIX
                + Resources.getString(
                    "Editor.RefreshPredefinedSetups.endWarnHeading", warningPds.size()));
        for (i = 0; i < warningPds.size(); i++)
          log(
              "|&nbsp; "
                  + Resources.getString(
                      "Editor.RefreshPredefinedSetups.endWarnPds",
                      warningPds.get(i).getAttributeValueString(PredefinedSetup.NAME),
                      "<b>" + warningPds.get(i).getFileName() + "</b>",
                      warningCount.get(i)));
      }
      ...
      if (!failPds.isEmpty()) {
        log(
            GameRefresher.ERROR_MESSAGE_PREFIX
                + Resources.getString(
                    "Editor.RefreshPredefinedSetups.endFailHeading", failPds.size()));
        for (i = 0; i < warningPds.size(); i++)
          log(
              "|&nbsp; "
                  + Resources.getString(
                      "Editor.RefreshPredefinedSetups.endFailPds",
                      failPds.get(i).getAttributeValueString(PredefinedSetup.NAME),
                      "<b>" + failPds.get(i).getFileName())
                  + "</b>");
      }

      log(
          Resources.getString(
              "Editor.RefreshPredefinedSetups.stats",
  ...
  /**
   * Checks the count of Pre-Defined Setups to be processed and displays appropriate message or
   * dialog box
   *
  ...
  private int promptConfirmCount(List<PredefinedSetup> pdsList) {
    ...
    return JOptionPane.showConfirmDialog(
                ...
                panel,
                Resources.getString(
                    "Editor.RefreshPredefinedSetups.confirm_title",
                    Resources.getString(
                        isTestMode()
                            ? "Editor.RefreshPredefinedSetups.confirm.test"
                            : "Editor.RefreshPredefinedSetups.confirm.run"),
                    i,
                    isFilterMode()
                        ? " " + Resources.getString("Editor.RefreshPredefinedSetups.confirm.filter")
                        : ""), //$NON-NLS-1$
  ...
  /**
   * Recursively collate all non-Container components inside a Container Ref: <a
   * href="https://stackoverflow.com/questions/2713425/java-swing-how-to-disable-a-jpanel">...</a>
   *
