...
public class PlayerInfoWindow extends JDialog {
  ...
  public PlayerInfoWindow(Frame f, SimplePlayer p) {
    ...
    final JLabel client_l = new JLabel(Resources.getString("Chat.client_version")); // $NON-NLS-1$
    ...
    if (moduleVersion != null && moduleVersion.length() > 0) {
      ...
      final JLabel mver_l = new JLabel(Resources.getString("Chat.module_version")); // $NON-NLS-1$
    ...
    if (csum != null && csum.length() > 0) {
      ...
      final JLabel csum_l = new JLabel(Resources.getString("Chat.module_checksum")); // $NON-NLS-1$
    ...
    if (c2sum != null && !c2sum.isEmpty()) {
      ...
      final JLabel csum_l =
          new JLabel(Resources.getString("Chat.combined_checksum")); // $NON-NLS-1$
      csum_l.setLabelFor(csum_f);
    ...
    final JLabel pinfo_l = new JLabel(Resources.getString("Chat.personal_info")); // $NON-NLS-1$
