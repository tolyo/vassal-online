 ...
 *
 * <p>This class handles the assorted problems with various versions of {@link ImageIO}, ensuring
 * that we can reliably load image files to {link BufferedImages} with a predictable type.
 *
 * @since 3.1.0
...
public class ImageIOImageLoader implements ImageLoader {
   ...
   *
   * @param tconv the <code>ImageTypeConverter</code> to use for type conversions
   */
  ...
  static {
    ...
    try (InputStream in =
        ImageIOImageLoader.class.getResourceAsStream("/images/black.jpg")) { // NON-NLS
  ...
  protected BufferedImage fix_tRNS(Reference<BufferedImage> ref, int tRNS, int type)
      throws ImageIOException {
  ...
  protected BufferedImage fix_grayscale_gamma(Reference<BufferedImage> ref, int type)
      throws ImageIOException {
