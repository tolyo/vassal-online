...
public class Board extends AbstractConfigurable implements GridContainer {
  ...
  /**
   * @return this <code>Board</code>'s {@link Map}. Until a game is started that is using this
   *     board, the map will be null.
   */
  public Map getMap() {
    return map;
  }
  ...
  public Class<?>[] getAttributeTypes() {
    return new Class<?>[] {
      String.class, Image.class, Boolean.class, Integer.class, Integer.class, Color.class
    };
  }
  ...
  private static final Comparator<Point> tileOrdering =
      (t1, t2) -> {
  ...
  public void drawRegion2D(
      ...
      if (zoom == 1.0 && !reversed) {
        ...
        if (scaledImageOp == null || scaledImageOp.getScale() != zoom) {
          if (boardImageOp instanceof SVGOp) {
            ...
            scaledImageOp =
                new FixedScaleOpTiledBitmapImpl(boardImageOp, zoom, bounds.width, bounds.height);
          } else {
            scaledImageOp =
                new FixedScaleOpBitmapImpl(boardImageOp, zoom, bounds.width, bounds.height);
          }
        }
    ...
    final Rectangle r =
        new Rectangle(
            visibleRect.x - lx, visibleRect.y - ly, visibleRect.width, visibleRect.height);
    ...
    for (final Point tile : tiles) {
      ...
      final Repainter rep = obs == null ? null : new Repainter(obs, cx, cy, cw, ch);

      try {
        final Future<BufferedImage> fim = op.getFutureTile(tile.x, tile.y, rep);
  ...
  public Point snapTo(Point p, boolean force, boolean onlyCenter) {
    return grid == null
        ? p
        : globalCoordinates(grid.snapTo(localCoordinates(p), force, onlyCenter));
  }
  ...
  /**
   * @return Position of this board relative to the other boards (0,0) is the upper left, (0,1) is
   *     to the right, etc.
   */
  ...
  public Rectangle bounds() {
    if (imageFile != null && boardImageOp != null && !fixedBoundaries) {
      ...
      if (magnification != 1.0) {
        boundaries.setSize(
            (int) Math.round(magnification * boundaries.width),
