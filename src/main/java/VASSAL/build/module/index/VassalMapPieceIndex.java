 ...
 * Maintain a Quadtree of x,y locations to enable fast direct lookup and ranged selections 2.
 * Maintain cross-references of all values for CurrentZone and LocationName to be able to quickly
 * find all pieces in a specified Zone, Region or module defined location like a Hex.
 */
public class VassalMapPieceIndex {
  ...
  /**
   * A property cross-reference of LocationName for fast lookup of pieces in a given region or board
   * position such as hex
   */
  ...
  public void addOrUpdatePiece(GamePiece piece) {
    ...
    if (pos != null) {
      ...
      // Then copy the existing Qtree to a new larger one that encompasses this piece, plus an extra
      // 100 pixels.
      // The Qtree is initially set to the size of the visible play area, so this should not happen
      // often.
  ...
  /**
   * Return a list of pieces in range of a given point. NOTE: Does not need to be accurate, may
   * include pieces out of range, range will be accurately checked later, but must not exclude
   * pieces that are in range
   *
   ...
   * @param point Point to search around
   * @param range Range in units appropriate to the search point
   * @param forceAsPixels Force the search to be in pixels, even if the target point is on a regular
   *     grid
   * @return List of pieces
   */
  public List<GamePiece> getPieces(Point point, int range, boolean forceAsPixels) {
    ...
    // Determine the pieces in range (ish). Pieces in the Qtree are stored in straight X,Y co-ords,
    // so need
    ...
    final List<GamePiece> rawPieces =
        qtree.getPiecesInRange(point, (range + 1) * pixelsPerRangeElement);
