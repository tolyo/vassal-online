...
public class DecoratorTest {
   ...
   * @param referenceTrait The reference Trait to be tested. The Reference trait does not need to
   *     have a BasicPiece as an Inner piece, but one will be added if it does not.
   */
  public void serializeTest(String test, Decorator referenceTrait)
  ...
  /**
   * Create and return a standard BasiCPiece trait with enough internals set up to be used for
   * simple tests
   *
  ...
  /**
   * Test that a trait's internal editor encodes the type in the same way that the trait started
   * with. Checks for serialization issues in the trait editors.
   *
   * <p>NOTE: Don't test State across the Editor, Trait Editors don't need to maintain state
   *
  ...
  public void editorTest(String test, Decorator referenceTrait) {
    ...
    assertThat(
        "Trait Edit Test (Type): " + test, editor.getType(), is(equalTo(originalType))); // NON-NLS
  }
  ...
  public void commandEncoderTest(String test, Decorator referenceTrait) {
    ...
    assertThat(
        "Command Encoder Test: " + test, referenceTrait.testEquals(testTrait), is(true)); // NON-NLS
  }

  /**
   * Test that a constructed Decorator with the type injected via its 2-arg constructor equals the
   * reference trait
   *
   * @param test Test Description
   * @param referenceTrait Reference trait for comparison
   */
  public void constructorTest(String test, Decorator referenceTrait)
      throws IllegalAccessException,
          InvocationTargetException,
          InstantiationException,
          NoSuchMethodException {
    // Build a new trait using the Type from the reference trait
    Constructor<? extends Decorator> constructor =
        referenceTrait.getClass().getConstructor(String.class, GamePiece.class);
    Decorator constructedTrait =
        constructor.newInstance(referenceTrait.myGetType(), createBasicPiece());
    ...
    assertThat(
        ...
        referenceTrait.testEquals(constructedTrait),
        is(true)); // NON-NLS
  }
}
