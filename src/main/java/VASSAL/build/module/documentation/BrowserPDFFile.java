...
/**
 * Easier-to-use support for opening a single PDF file in the browser (it can be done with
 * BrowserHelpFile, but requires a substantial workaround and weird confusing procedures)
 */
public class BrowserPDFFile extends AbstractConfigurable {
  ...
  protected void extractPDF() {
    ...
    try (InputStream in = archive.getInputStream(pdfFile)) {
      final Path out =
          Files.createTempFile(Info.getTempDir().toPath(), "pdfhelp_", ".pdf"); // NON-NLS
      try {
