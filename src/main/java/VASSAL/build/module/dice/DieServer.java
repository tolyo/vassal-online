...
/**
 * Base DieServer Class Does most of the work. Individual Die Servers just need to implement {@link
 * #buildInternetRollString} and {@link #parseInternetRollString} methods.
 */
    ...
    for (final DieRoll roll : rolls) {
      ...
      for (int j = 0; j < nDice; j++) {
        ...
        if (reportTotal) {
          ...
          val.append(result);
          if (j < nDice - 1) val.append(',');
        }
      }

      if (reportTotal) val.append(total);

      GameModule.getGameModule()
          .getChatter()
          .send(formatResult(roll.getDescription(), val.toString(), format));
    }
  }
