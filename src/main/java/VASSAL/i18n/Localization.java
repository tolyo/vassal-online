...
public class Localization extends Language {
   ...
   *
   * @param component component to be translated
   * @param name Attribute name to be translated
  ...
  /**
   * Translate the module. The module and all extensions have now been built, so all Translations
   * are available and all attributes that need to be translated have been recorded. There may be
   * multiple translations that match this Locale, merge them in order - Country overrides Language
   * overrides default. NB - You cannot create a default translation (Module.properties) using the
   * VASSAL editor, but a default file can be placed into a module or extension manually.
   *
  ...
  public void translate() throws IOException {
    if (GameModule.getGameModule().isLocalizationEnabled()) {
      ...
      translationComplete =
          true; // Ensure Translation marked complete even if no actual translation took place
    }
  }
   ...
   * @param defaultValue Default value if no translation available
   * @return translation
   */
  ...
  /**
   * Called whenever a Translation is added to a module or extension. Check if the translation
   * matches our locale. If so, add it to the list of translations to use. There may multiple
   * matching translations at Country, Language and Module level from different extensions.
   *
   * @param t Translation
   */
