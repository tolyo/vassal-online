...
/**
 * StackMetrics provides the [Stacking options] component of a {@link Map}. It encapsulates
 * information on how to draw expanded and unexpanded views of a stack.
 */
public class StackMetrics extends AbstractConfigurable {
    ...
    if (EXSEP_X.equals(name)) {
      if (value instanceof String) {
        try {
          ...
          ErrorDialog.dataWarning(
              new BadDataReport(
                  Resources.getString("Error.bad_preference", EXSEP_X, "StackMetrics"),
                  (String) value,
                  NaN)); // NON-NLS
        }
      } else if (value != null) {
      ...
      if (value instanceof String) {
        try {
          ...
          ErrorDialog.dataWarning(
              new BadDataReport(
                  Resources.getString("Error.bad_preference", EXSEP_Y, "StackMetrics"),
                  (String) value,
                  NaN)); // NON-NLS
        }
      } else if (value != null) {
      ...
      if (value instanceof String) {
        try {
          ...
          ErrorDialog.dataWarning(
              new BadDataReport(
                  Resources.getString("Error.bad_preference", UNEXSEP_X, "StackMetrics"),
                  (String) value,
                  NaN)); // NON-NLS
        }
      } else if (value != null) {
      ...
      if (value instanceof String) {
        try {
          ...
          ErrorDialog.dataWarning(
              new BadDataReport(
                  Resources.getString("Error.bad_preference", UNEXSEP_Y, "StackMetrics"),
                  (String) value,
                  NaN)); // NON-NLS
        }
      } else if (value != null) {
  ...
  public String getAttributeValueString(String name) {
    if (EXSEP_X.equals(name)) {
      ...
      return blankColor == null ? null : ColorConfigurer.colorToString(blankColor);
    }
    return null;
  }
  ...
  /**
   * Different instances of StackMetrics may render a {@link Stack} in different ways. The default
   * algorithm is: If not expanded, all but the top visible GamePiece is drawn as a white square
   * with size given by {@link GamePiece#getShape}. The separation between GamePieces is given by
   * {@link #relativePosition}
   *
   * <p>If expanded, all GamePieces are drawn with separation given by {@link #relativePosition}.
   * GamePiece that are selected are drawn in front of other GamePieces, even those above them in
   * the stack.
   */
  public void draw(Stack stack, Graphics g, int x, int y, Component obs, double zoom) {
    final Highlighter highlighter =
        stack.getMap() == null ? BasicPiece.getHighlighter() : stack.getMap().getHighlighter();
    final Point[] positions = new Point[stack.getPieceCount()];
    getContents(stack, positions, null, null, x, y);

    for (final PieceIterator e = new PieceIterator(stack.getPiecesIterator(), unselectedVisible);
        e.hasMoreElements(); ) {

      final GamePiece next = e.nextPiece();
      final int index = stack.indexOf(next);
      if (index >= 0) { // BR// Bounds-check index as a bandaid against getting drawn during e.g. a
        // screenshot or loadgame
    ...
    stack.asList().stream()
        ...
        .forEach(
            gamePiece -> {
              ...
              if (index
                  >= 0) { // BR// Bounds-check index as a bandaid against getting drawn during e.g.
                // a screenshot or loadgame
                final int nextX = x + (int) (zoom * (positions[index].x - x));
                final int nextY = y + (int) (zoom * (positions[index].y - y));
  ...
  /**
   * Draw only those pieces in the target stack whose boundingBoxes fall within the given
   * visibleRect This method is considerably faster than the other draw method.
   *
   * @param stack
   * @param g
            ...
            gamePiece -> {
              ...
              if ((index >= 0)
                  && (bounds == null
                      || isVisible(
                          region,
                          bounds[index]))) { // BR// Bounds-check index as a bandaid against getting
                // drawn during e.g. a screenshot or loadgame
                final Point pt = map.mapToDrawing(positions[index], os_scale);
   ...
   *
   * <p>Default implementation is a white square with a black border
   */
  ...
  /**
   * The color used to draw boxes representing counters beneath the top one in a stack. A value of
   * null indicates that the counters should be drawn fully
   *
   * @return
   */
  ...
  /**
   * Fill the argument arrays with the positions, selection bounds and bounding boxes of the pieces
   * in the argument stack
   *
   * @param parent The parent Stack
   * @param positions If non-null will contain a {@link Point} giving the position of each piece in
   *     <code>parent</code>
   * @param shapes If non-null will contain a {@link Shape} giving the shape of for each piece in
   *     <code>parent</code>
   * @param boundingBoxes If non-null will contain a {@link Rectangle} giving the bounding box for
   *     each piece in <code>parent</code>
  ...
  public int getContents(
      Stack parent, Point[] positions, Shape[] shapes, Rectangle[] boundingBoxes, int x, int y) {
  ...
  public Point relativePosition(Stack parent, GamePiece c) {
    final int index = Math.min(parent.indexOf(c), parent.getMaximumVisiblePieceCount() - 1);
  ...
  public String[] getAttributeNames() {
    return new String[] {
      DISABLED, EXSEP_X, EXSEP_Y, UNEXSEP_X, UNEXSEP_Y, COLOR, TOP_KEY, BOTTOM_KEY, UP_KEY, DOWN_KEY
    };
  }
  ...
  public Class<?>[] getAttributeTypes() {
    return new Class<?>[] {
      Boolean.class, Integer.class, Integer.class, Integer.class, Integer.class, Color.class
    };
  }
  ...
  /**
   * Merge the two pieces if stacking is enabled. If stacking is disabled, place the moving piece at
   * the same location as the fixed piece
   *
   * @param fixed
   * @param moving
  ...
  /**
   * Place a GamePiece on top of another GamePiece Create/remove stacks as necessary, even if
   * stacking is disabled for this instance
   *
    ...
    if (fixed instanceof Stack
      ...
      if (moving != fixed && moving != fixedParent) {
        ...
        if (moving instanceof Stack) {
          ...
          if (moving.getParent() == fixedParent
              && fixedParent != null
              && fixedParent.indexOf(moving) < index) {
            index--;
          }
