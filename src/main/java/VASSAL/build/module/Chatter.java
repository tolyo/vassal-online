...
/**
 * The chat window component. Displays text messages and accepts input. Also acts as a {@link
 * CommandEncoder}, encoding/decoding commands that display message in the text area
 */
public class Chatter extends JPanel implements CommandEncoder, Buildable, DropTargetListener {
  ...
  protected static final String MY_CHAT_COLOR =
      "HTMLChatColor"; //$NON-NLS-1$ // Different tags to "restart" w/ new default scheme
  ...
  protected JTextArea
      conversation; // Backward compatibility for overridden classes. Needs something to suppress.
  ...
  public Chatter() {
    ...
    // BR// Conversation is now a JTextPane w/ HTMLEditorKit to process HTML, which gives us HTML
    // support "for free".
    ...
    myFont =
        new Font(
            "SansSerif",
            Font.PLAIN,
            12); // NON-NLS // Will be overridden by the font from Chat preferences

    try {
      for (int i = 0;
          i < 30;
          ++i) { // Scroll past some arbitrary number of empty lines so that first text will start
    ...
    conversationPane.addComponentListener(
    ...
    input.setMaximumSize(
        new Dimension(input.getMaximumSize().width, input.getPreferredSize().height));
    ...
    scroll
        ...
        .setUnitIncrement(input.getPreferredSize().height); // Scroll this faster

    final int fontHeight = getFontMetrics(myFont).getHeight();
    setPreferredSize(
        new Dimension(
            input.getMaximumSize().width, input.getPreferredSize().height + 10 * fontHeight));
    ...
    final Action saveAction =
        new AbstractAction(Resources.getString("Chat.save_to_html_file")) {
          @Override
          public void actionPerformed(ActionEvent e) {
            final FileChooser fc = GameModule.getGameModule().getFileChooser();

            if (fc.showSaveDialog(GameModule.getGameModule().getControlPanel())
    ...
    final Action saveTextAction =
        new AbstractAction(Resources.getString("Chat.save_to_text_file")) {
          @Override
          public void actionPerformed(ActionEvent e) {
            final FileChooser fc = GameModule.getGameModule().getFileChooser();
            ...
            try {
              FileUtils.writeStringToFile(
                  outputFile, doc.getText(0, doc.getLength()), StandardCharsets.UTF_8); // NON-NLS
            } catch (IOException | BadLocationException ex) {
              GameModule.getGameModule().warn(Resources.getString("Chat.file_save_failed"));
            }
          }
    ...
    conversationPane.addMouseListener(
        new MouseAdapter() {
          @Override
  ...
  /**
   * Because our Chatters make themselves visible in their constructor, providing a way for an
   * overriding class to "turn this chatter off" is friendlier than What Went Before.
   *
  ...
  protected String formatChat(String text) {
    ...
    return String.format(
        "&lt;%s&gt; - %s",
        id.isEmpty() ? "(" + getAnonymousUserName() + ")" : id,
  ...
  /**
   * Styles a chat message based on the player who sent it. Presently just distinguishes a chat
   * message "from me" from a chat message "from anyone else".
   *
   * <p>To make the player colors easy to override in a custom class (my modules have logic to
   * assign individual player colors -- beyond the scope of the present effort but a perhaps a fun
   * future addition)
   *
   ...
   * @param html_allowed - flag if html_processing is enabled for this message (allows console to
  ...
  /**
   * Display a message in the text area - use show() from outside the class - MUST run on
   * EventDispatchThread
   */
  private void doShow(String s) {
    ...
    if (!s.isEmpty()) {
      if (s.startsWith("*")) {
        ...
        style =
            (QuickColors.getQuickColor(s, "-") >= 0)
                ? QuickColors.getQuickColorHTMLStyle(s, "-")
                : "sys"; // NON-NLS
        s = QuickColors.stripQuickColorTag(s, "-");
      } else {
    ...
    // (2) Legacy module "report" text when not explicitly opted in w/ first character or preference
    // setting
    if (!html_allowed) {
      s =
          s.replaceAll("<", "&lt;") // NON-NLS // This prevents any unwanted tag from functioning
              .replaceAll(
                  ">",
  ...
  /**
   * Expects to be added to a GameModule. Adds itself to the controls window and registers itself as
   * a {@link CommandEncoder}
   */
  @Override
  public void addTo(Buildable b) {
    ...
    // game message color
    //
    final ColorConfigurer gameMsgColor =
        new ColorConfigurer(
            GAME_MSG1_COLOR, Resources.getString("Chatter.game_messages_preference"), Color.black);

    gameMsgColor.addPropertyChangeListener(
        e -> {
    ...
    final ColorConfigurer gameMsg2Color =
        new ColorConfigurer(
            GAME_MSG2_COLOR,
            Resources.getString("Chatter.game_messages_preference_2"),
            new Color(0, 153, 51));

    gameMsg2Color.addPropertyChangeListener(
        e -> {
    ...
    final ColorConfigurer gameMsg3Color =
        new ColorConfigurer(
            GAME_MSG3_COLOR,
            Resources.getString("Chatter.game_messages_preference_3"),
            new Color(255, 102, 102));

    gameMsg3Color.addPropertyChangeListener(
        e -> {
    ...
    final ColorConfigurer gameMsg4Color =
        new ColorConfigurer(
            GAME_MSG4_COLOR,
            Resources.getString("Chatter.game_messages_preference_4"),
            new Color(255, 0, 0));

    gameMsg4Color.addPropertyChangeListener(
        e -> {
    ...
    final ColorConfigurer gameMsg5Color =
        new ColorConfigurer(
            GAME_MSG5_COLOR,
            Resources.getString("Chatter.game_messages_preference_5"),
            new Color(153, 0, 153));

    gameMsg5Color.addPropertyChangeListener(
        e -> {
    ...
    final ColorConfigurer systemMsgColor =
        new ColorConfigurer(
            SYS_MSG_COLOR,
            Resources.getString("Chatter.system_message_preference"),
            new Color(160, 160, 160));

    systemMsgColor.addPropertyChangeListener(
        e -> {
    ...
    final ColorConfigurer myChatColor =
        new ColorConfigurer(
            MY_CHAT_COLOR,
            Resources.getString("Chatter.my_text_preference"),
            new Color(9, 32, 229));

    myChatColor.addPropertyChangeListener(
        e -> {
    ...
    final ColorConfigurer otherChatColor =
        new ColorConfigurer(
            OTHER_CHAT_COLOR,
            Resources.getString("Chatter.other_text_preference"),
            new Color(0, 153, 255));

    otherChatColor.addPropertyChangeListener(
        e -> {
  ...
  /**
   * Displays the message, Also logs and sends to the server a {@link Command} that displays this
   * message.
   */
  ...
  /**
   * Warning message method -- same as send, but accepts messages from static methods. For reporting
   * soft-fail problems in modules.
   */
  ...
  public void keyCommand(KeyStroke e) {
    ...
    if ((e.getKeyCode() == 0 || e.getKeyCode() == KeyEvent.CHAR_UNDEFINED)
      ...
      if ((e.getModifiers()
              & (KeyEvent.ALT_DOWN_MASK | KeyEvent.CTRL_DOWN_MASK | KeyEvent.ALT_GRAPH_DOWN_MASK))
          != 0) {
  ...
  /**
   * This is a {@link Command} object that, when executed, displays a text message in the Chatter's
   * text area
   */
  public static class DisplayText extends Command {
    ...
    public DisplayText(Chatter c, String s) {
      ...
      if (s.startsWith("<>")) { // NON-NLS
        msg =
            "&lt;(" + getAnonymousUserName() + ")&gt;" + s.substring(2); // HTML-friendly //NON-NLS
        // angle brackets
      } else {
        msg = s;
      }
