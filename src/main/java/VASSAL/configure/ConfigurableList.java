...
/**
 * An interface to be implemented by Configurers that manage variable sized lists. See also {@link
 * ConfigurableListEntry}, {@link ConfigurableListController}
 */
public interface ConfigurableList {
  ...
  /**
   * Move the currently selected list entry up one position The moved entry should retain the
   * selection
   */
  void moveEntryUp();

  /**
   * Move the currently selected list entry down one position The moved entry should retain the
   * selection
   */
  void moveEntryDown();

  /**
   * Add a new list entry after the currently selected entry, or at the bottom of the list if no
   * entries selected. The new entry should be selected.
   */
  void addEntry();

  /**
   * Remove the specified entry from the list. The entry following the removed entry should be
   * selected, or the last entry in the list if the deleted entry was last.
   *
