...
public class ActionButton extends Decorator implements EditablePiece, Loopable {
  ...
  protected boolean launchPopupMenu =
      false; // BR// If clicking this button should launch the piece's context menu
  protected boolean useWholeShape =
      false; // BR// If we should just use the whole piece shape as the hotspot for the button

  public ActionButton() {
    this(ID, null);
    // this(ID + "65,0;-20;-20;40;40;;false,true;1;0", null); //NON-NLS // Starts new ones as
    // "version 1"
  }
  ...
  public String myGetType() {
    ...
    for (int point = 0; point < polygon.npoints; point++) {
      // Point-by-point instead of e.g. all-x-first makes it easier to build the polygon at the
      // other end
  ...
  public void mySetType(String type) {
    ...
    polygon.reset();
    if (version < 1) {
      // If it's a genuine old-format then convert the old bounds over; otherwise start with empty
      // polygon and use "whole piece" by default
  ...
  public static class VisualEditor implements PolygonConfigurer {
    ...
    private Point
        offsetView; // We need to offset the coordinate space in the PolygonEditor so that 0,0 will
    // be at the center of our coordinate space rather than the upper left corner.
    ...
    protected final JLabel warning =
        new JLabel(Resources.getString("Editor.ActionButton.empty_polygon"));
    ...
    public void show(ActionButton piece, Polygon poly, Ed pieceEditor) {
      ...
      // If we don't have a legitimate polygon right now, see if we can create a default starting
      // one from the bounds of our piece
      ...
      // Define a reasonable sized coordinate space, but big enough for our (a) piece bounding box
      // if any, (b) current size of hotspot polygon if any, and (c) 500x500 minimum
      ...
      editor =
          new PolygonEditor(
              new Polygon(polygon.xpoints, polygon.ypoints, polygon.npoints), offsetView) {
            ...
            protected void paintBackground(Graphics g) {
              ...
              warning.setVisible(
                  editor != null
                      && (editor.getPolygon() == null || editor.getPolygon().npoints == 0));
            }
          };
      ...
      final JButton direct =
          new JButton(Resources.getString("Editor.ActionButton.set_coordinates_directly"));
          ...
          e -> {
            String newShape =
                JOptionPane.showInputDialog(
                    frame,
                    Resources.getString("Editor.ActionButton.enter_points_instructions"),
            ...
            if (newShape != null) {
              ...
              editor.setPolygon(
                  newPoly); // Need to send it back in this way to get our offsetView respected
              editor.repaint();
            }
          });
      ...
      if (editor.getPolygon() != null) {
        ...
        final Point polyCenter =
            new Point(
                currentBounds.x + currentBounds.width / 2 - offsetView.x,
      ...
      frame.setSize(
          Math.min(frame.getWidth(), d.width * 2 / 3),
          Math.min(frame.getHeight(), d.height * 2 / 3));
  ...
  public static class Ed implements PieceEditor {
    ...
    public Ed(ActionButton p) {
      ...
      useShapeConfig.addPropertyChangeListener(
  ...
  public static List<GamePiece> matchingTraits(GamePiece p, Point point) {
    ...
    // If we're suppressing action buttons on recently moved pieces, ignore click on this piece if
    // it's selected
    final KeyBuffer kb = KeyBuffer.getBuffer();
    if (kb.isSuppressActionButtons() && kb.contains(p)) return results;

    for (GamePiece trait = p; trait instanceof Decorator; trait = ((Decorator) trait).getInner()) {
      if (trait instanceof Obscurable) {
        ...
        // If we're not a use-whole-shape piece, then check this click against the special
        // designated polygon
        if (!action.useWholeShape) {
          ...
          if (cumulative != 0.0) {
            shape =
                AffineTransform.getRotateInstance(cumulative, 0, 0)
  ...
  /**
   * Registers mouse listeners with Maps and other components. Clicking the mouse checks for pieces
   * with an ActionButton trait and invokes them if the click falls within the button's boundaries
   */
  protected static class ButtonPusher {
    private final Set<Map> maps = new HashSet<>();
    private final java.util.Map<Component, ComponentMouseListener> componentMouseListeners =
        new HashMap<>();
    ...
    /**
     * Handle a mouse click on the given GamePiece at the given location (where 0,0 is the center of
     * the piece). Activate all Action Buttons in sequence that are not Masked or Hidden
     *
     * @param p A function to determine the offset of the target piece. This callback is done for
     *     efficiency reasons, since computing the offset may be expensive (as in the case of a
     *     piece in an expanded stack on a map) and is only needed if the piece has the ActionButton
     *     trait
     */
