...
public class ADC2Utils {
  ...
  public static final Color[] defaultColorPallet = {
    new Color(0x000000),
    new Color(0x808080),
    new Color(0x800000),
    new Color(0x808000),
  ...
  /**
   * Read a base-250 2-byte big-endian word from a <code>DataInputStream</code>. This is the default
   * (and only) encoding for words imported modules.
   */
  ...
  /**
   * Read a base-250 4-byte big-endian word from a <code>DataInputStream</code>. This is the default
   * encoding for integers in ADC2 modules (why?).
   */
  ...
  /**
   * Read a block separator byte from an import module file and throw an exception if it doesn't
   * match.
   */
  static void readBlockHeader(DataInputStream in, String string) throws IOException {
    ...
    if (in.available() == 0) throw new NoMoreBlocksException(string);
  }
