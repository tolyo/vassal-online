...
/**
 * The IndexManager maintains a set of indexes on GamePieces that will primarily be used by the GKC
 * Fast-match mechanism to quickly find sets of pieces that meet certain critera.
 *
 * <p>It maintains the following indexes: o A per-map quadtree of piece locations o A per-map
 * cross-reference of values of the Currentzone and LocationName properties
 *
 * <p>The indexes for each Map are stored in a VassalMapPieceIndex object
 *
 * <p>NOTE: Pieces not on Maps are NOT included in any index.
 */
public class IndexManager {
  ...
  /**
   * Clear and rebuild all indexes using pieces currently existing on boards. Will usually only be
   * called at the start of a game session after initial load of gamestate is complete.
   */
  public void rebuild() {
    clearAll();
  ...
  /**
   * A piece has just moved or been placed on the board, update any location based indexes and
   * create any property based indexes if we have not seen this piece before
   *
   ...
   * @param forceAsPixels Force the range check to be in pixels, overriding any grid at the target
   *     point
