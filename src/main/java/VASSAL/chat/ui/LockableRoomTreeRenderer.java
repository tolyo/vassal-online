...
public class LockableRoomTreeRenderer extends RoomTreeRenderer {
  ...
  public Component getTreeCellRendererComponent(
    ...
    if (item instanceof LockableRoom) {
      ...
      final DefaultMutableTreeNode roomNode =
          (DefaultMutableTreeNode) ((DefaultMutableTreeNode) value).getParent();
      final Object room = roomNode.getUserObject();
      if (room instanceof LockableRoom) {
        if (!ChatServerConnection.DEFAULT_ROOM_NAME.equals(((Room) room).getName())
            && ((LockableRoom) room).isOwner(((Player) item).getId())) {
          if (ownerFont == null) {
            nonOwnerFont = this.getFont();
            ownerFont =
                new Font(
                    nonOwnerFont.getFontName(),
                    nonOwnerFont.getStyle() + Font.BOLD,
                    nonOwnerFont.getSize());
          }
          setFont(ownerFont);
          return l;
        }
