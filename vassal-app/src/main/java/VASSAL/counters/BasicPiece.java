...
/**
 * Basic class for representing a physical component of the game. Can be e.g. a counter, a card, or
 * an overlay.
 *
 * <p>Note like traits, BasicPiece implements GamePiece (via TranslatablePiece), but UNLIKE traits
 * it is NOT a Decorator, and thus must be treated specially.
 */
public class BasicPiece extends AbstractImageFinder
   ...
   *
   * <p>LocationName - Current Location Name of piece as displayed in Chat Window CurrentX - Current
   * X position CurrentY - Current Y position CurrentMap - Current Map name or "" if not on a map
   * CurrentBoard - Current Board name or "" if not on a map CurrentZone - If the current map has a
   * multi-zoned grid, then return the name of the Zone the piece is in, or "" if the piece is not
  ...
  private String
      commonName; // BasicPiece's name for the piece (aka "BasicName" property in Vassal Module)
   ...
   *
   * @param type serialized type information (data about the piece which does not change during the
   *     course of a game) ready to be processed by a {@link SequenceEncoder.Decoder}
   */
  ...
  /**
   * @return The "type information" of a piece or trait is information that does not change during
   *     the course of a game. Image file names, context menu strings, etc., all should be reflected
   *     in the type. The type information is returned serialized string form, ready to be decoded
   *     by a SequenceEncoder#decode.
   ...
   * #getState} method.
   *
   * <p>A request to getProperty() that reaches the BasicPiece will have already checked for such a
   * property key being available from any outer Decorator/Trait in the stack. Upon reaching
   * BasicPiece, the search hierarchy for a matching property now becomes:
   *
   * <p>(1) Specific named properties supported by BasicPiece. These include BASIC_NAME, PIECE_NAME,
   * LOCATION_NAME, CURRENT_MAP, CURRENT_BOARD, CURRENT_ZONE, CURRENT_X, CURRENT_Y. (2) "Scratchpad"
   * properties - see {@link #setProperty} for full details, but these are highly temporary
   * properties intended to remain valid only during the execution of a single key command. (3)
   * Persistent properties - see {@link #setPersistentProperty} for full details, but they are
   * stored in the piece and "game state robust" - saved during save/load, and propagated to other
   * players' clients in a multiplayer game. (4) The values of any visible "Global Property" in a
   * Vassal module, checking the Zone level first, then the map level, and finally the module level.
   * <br>
   * <br>
   * Thus, when using this interface a piece's own properties are preferred to those of "Global
   * Properties", and those in turn are searched Zone-first then Map, then Module.
   *
  ...
  public Object getProperty(Object key) {
    ...
    } else return getPublicProperty(key);
  }

  /**
   * Properties (see {@link #getProperty}) visible in a masked (see {@link Obscurable}) piece, even
   * when the piece is masked.
   *
   * @param key String key of property to be returned.
   */
  public Object getPublicProperty(Object key) {
    ...
    // Check for higher level properties. Each level if it exists will check the higher level if
    // required.
    if (prop == null) {
  ...
  /**
   * Returns the localized text for a specified property if a translation is available, otherwise
   * the non-localized version. Searches the same hierarchy of properties as {@link #getProperty}.
   *
   * @param key String key of property to be returned
  ...
  /**
   * Returns the localized text for a specified property if a translation is available, otherwise
   * the non-localized version, but in both cases accounting for the unit's visibility (i.e.
   * Mask/{@link Obscurable} Traits). Searches the same hierarchy of properties as {@link
   * #getProperty}.
   *
   * @param key String key of property to be returned
   * @return Returns localized text of property, if available, otherwise non-localized value,
   *     accounting for Mask status.
   */
   ...
   * #getState} method.
   *
   * <p>A setProperty() call which reaches BasicPiece will already have passed through all of the
   * outer Decorator/Traits on the way in without finding one able to match the property. <br>
   * <br>
   ...
   * are not perfect mirrors. This method ALSO does not set persistent properties (they can only be
   * set by an explicit call to {@link #setPersistentProperty}). <br>
   * <br>
   * BasicPiece <i>does</i>, however contain a "scratchpad" for temporary properties, and for any
   * call to this method that does not match a known property (which is, currently, ANY call which
   * reaches this method here in BasicPiece), a scratchpad property will be set. Scratchpad
   * properties are NOT saved when the game is saved, and NO arrangement is made to pass their
   * values to other players' machines. Thus they should only be used internally for highly
   * temporary values during the execution of a single key command. Their one other use is to store
   * the piece's Unique ID -- and although this value is obviously used over periods of time much
   * longer than a single key command, this is possible because the value is immutable and is
   * refreshed to the same value whenever the piece is re-created e.g. when loading a save.
   *
  ...
  /**
   * Setting a persistent property writes a property value into the piece (creating a new entry in
   * the piece's persistent property table if the specified key does not yet exist in it).
   ...
   * property value can then be read from the piece via e.g. {@link #getProperty}. When reading back
   * properties out of a piece, the piece's built-in properties are checked first, then scratchpad
   * properties (see {@link #setProperty}), then external properties such as Global Properties. If
   ...
   * Persistent Property is indeed nearly identical with {@link DynamicProperty} in
   * storage/retrieval characteristics, and simply lacks the in-module interface for setting values,
   * etc. Module Designers are thus recommended to stick with Dynamic Property traits for these
   * functions.
   *
  ...
  public Command setPersistentProperty(Object key, Object newValue) {
    final Object oldValue =
        newValue == null
            ? getPersistentProps().remove(key)
            : getPersistentProps().put(key, newValue);
    return Objects.equals(oldValue, newValue)
        ? null
        : new SetPersistentPropertyCommand(getId(), key, oldValue, newValue);
  }
  ...
  public void draw(Graphics g, int x, int y, Component obs, double zoom) {
    ...
    imagePainter.draw(
        g, x + (int) (zoom * imageBounds.x), y + (int) (zoom * imageBounds.y), zoom, obs);
  }

  /**
   * @return the set of key commands that will populate the a BasicPiece's right-click menu. This
   *     will normally be an empty array in the present age of the world, but the ability to contain
   *     a clone and delete command is retained for compatibility with Modules Of Ancient Times. In
   *     the case of BasicPiece, this method also keeps track of whether move
   *     up/down/to-top/to-bottom commands are enabled.
   *     <p>This method is chained from "outer" Decorator components of a larger logical game piece,
   *     in the process of generating the complete list of key commands to build the right-click
   *     menu -- this process is originated by calling <code>getKeyCommands()</code> on the piece's
   *     outermost Decorator/Trait.
   */
  protected KeyCommand[] getKeyCommands() {
    if (commands == null) {
      ...
      if (cloneKey > 0) {
        l.add(
            new KeyCommand(
                Resources.getString("Editor.Clone.clone"),
                KeyStroke.getKeyStroke(cloneKey, InputEvent.CTRL_DOWN_MASK),
                target));
      }
      if (deleteKey > 0) {
        l.add(
            new KeyCommand(
                Resources.getString("Editor.Delete.delete"),
                KeyStroke.getKeyStroke(deleteKey, InputEvent.CTRL_DOWN_MASK),
                target));
      }
    ...
    // boolean canAdjustPosition = outer.getMap() != null && outer.getParent() != null &&
   ...
   * @return false if no Keystroke, true if not an internal-to-BasicPiece key command, and enabled
   *     status of key command otherwise.
   */
  ...
  /**
   * @return the parent {@link Stack} of which this piece is a member, or null if not a member of
   *     any Stack
   */
  @Override
  ...
  /**
   * @return the Shape of this piece, for purposes of selecting it by clicking on it with the mouse.
   *     In the case of BasicPiece, this is equivalent to the boundingBox of the BasicPiece image,
   *     if one exists. Note that the shape should be defined in reference to the piece's location,
   *     which is ordinarily the center of the basic image. <br>
   *     <br>
   *     For pieces that need a non-rectangular click volume, add a {@link NonRectangular} trait.
   */
  @Override
  ...
  /**
   * @return the name of this BasicPiece. This is the name typed by the module designer in the
   *     configuration box for the BasicPiece.
   */
  ...
  /**
   * @return the name of this GamePiece. It queries the BasicName property from the outermost piece
   *     inward, allowing a "Basic Name" trait to possibly override ours when the Piece Name is
   *     being queried.
   */
  @Override
  ...
  /**
   * @return the localized name of this BasicPiece. This is the translated version of the name typed
   *     by the module designer in the configuration box for the BasicPiece. It is used to fill the
   *     "BasicName" property if the search for it makes it all the way to this innermost piece.
   */
  ...
  /**
   * @return the localized name of this GamePiece. It queries the BasicName property from the
   *     outermost piece inward, allowing a "Basic Name" trait to possibly override ours when the
   *     Piece Name is being queried.
   */
  @Override
   ...
   * to this method if they are received while the piece is selected (or as the result of e.g. a
   * Global Key Command being sent to the piece). The class implementing GamePiece can respond in
   * any way it likes. Actual key presses by the player, selected items from the right-click Context
   * Menu, keystrokes "applied on move" by a Map that the piece has just moved on, and Global Key
   * Commands all send KeyStrokes (and NamedKeyStrokes) which are passed to pieces and traits
   * through this interface. <br>
   * <br>
   * In the case of BasicPiece, if a key command gets here, that means it has already been seen by
   * any and all of its Traits ({@link Decorator}s), as BasicPiece is the innermost member of the
   * Decorator stack. The key events processed here by BasicPiece include the "move up"/"move
   * down"/"move-to-top"/"move-to-bottom" stack-adjustment commands, along with legacy support for
   * cloning and deleting.
   *
   * @return a {@link Command} that, when executed, will make all changes to the game state (maps,
   *     pieces, other pieces, etc) to duplicate what the piece did in response to this event on
   *     another machine. Often a {@link ChangePiece} command, but for example if this keystroke
   *     caused the piece/trait to decide to fire off a Global Key Command, then the Command
   *     returned would include the <i>entire</i> results of that, appended as subcommands.
  ...
  public Command keyEvent(KeyStroke stroke) {
    ...
    final GamePiece outer = Decorator.getOutermost(this);
    if (cloneKey != 0
        && KeyStroke.getKeyStroke(cloneKey, InputEvent.CTRL_DOWN_MASK).equals(stroke)) {
      ...
      if (getMap() != null) {
        ...
        && KeyStroke.getKeyStroke(deleteKey, InputEvent.CTRL_DOWN_MASK).equals(stroke)) {
  ...
  /**
   * @return The "state information" is information that can change during the course of a game.
   *     State information is saved when the game is saved and is transferred between players on the
   *     server. For example, the relative order of pieces in a stack is state information, but
   *     whether the stack is expanded is not. <br>
   *     <br>
   *     In the case of BasicPiece, the state information includes the current map, x/y position,
   *     the unique Game Piece ID, and the keys and values of any persistent properties (see {@link
  ...
  /**
   * @param s New state information serialized in string form, ready to be passed to a
   *     SequenceEncoder#decode. The "state information" is information that can change during the
   *     course of a game. State information is saved when the game is saved and is transferred
   *     between players on the server. For example, the relative order of pieces in a stack is
   *     state information, but whether the stack is expanded is not. <br>
   *     <br>
   *     In the case of BasicPiece, the state information includes the current map, x/y position,
   *     the unique Game Piece ID, and the keys and values of any persistent properties (see {@link
   *     #setPersistentProperty})
  ...
  public void setState(String s) {
    ...
    if (!"null".equals(mapId)) { // NON-NLS
      ...
      if (newMap == null) {
        Decorator.reportDataError(
            this, Resources.getString("Error.not_found", "Map"), "mapId=" + mapId); // NON-NLS
      }
    }
    ...
    // Maintain the value of UNIQUE_ID, as it will have been defaulted for legacy units when they
    // are first
  ...
  /**
   * For BasicPiece, the "merge" of a new state simply involves copying in the new one in its
   * entirety -- if any difference is detected.
   *
  ...
  /**
   * Each GamePiece must have a unique String identifier. These are managed by VASSAL internally and
   * should never be changed by custom code.
   *
  ...
  /**
   * @return the Highlighter instance for drawing selected pieces. Note that since this is a static
   *     method, all pieces in a module will always use the same Highlighter
   */
  ...
  /**
   * @return The configurer ({@link PieceEditor} for the BasicPiece, which generates the dialog for
   *     editing the BasicPiece's type information in the Editor window.
   */
  @Override
  ...
  /**
   * Test if this BasicPiece's Type and State are equal to another This method is intended to be
   * used by Unit Tests to verify that a trait is unchanged after going through a process such as
  ...
  /**
   * The configurer ({@link PieceEditor} for the BasicPiece, which generates the dialog for editing
   * the BasicPiece's type information in the Editor window.
   */
  private static class Ed implements PieceEditor {
    ...
    /**
     * @return the type information string for the BasicPiece based on the current values of the
     *     configurer fields
     */
    @Override
    public String getType() {
      ...
      final String type =
          se.append(deleteKeyInput.getKey())
              .append(picker.getValueString())
              .append(pieceName.getValueString())
              .getValue();
      return ID + type;
    }
  }
  ...
  /**
   * @return Property names exposed by the Trait or Piece. In the case of BasicPiece, there are
   *     quite a few, mainly dealing with past and present location.
   */
  @Override
  ...
  /**
   * See {@link AbstractImageFinder} Adds our image (if any) to the list of images
   *
