...
public class FirstTimeDialog extends JDialog {
  ...
  public FirstTimeDialog(Frame parent) {
    ...
    final JButton jump = new JButton(Resources.getString("Main.jump_right_in")); // $NON-NLS-1$
    ...
    if (img != null) {
      ...
      final Dimension remainder =
          new Dimension(
              Math.max(screen.width - dsize.width, 0), Math.max(screen.height - dsize.height, 0));
