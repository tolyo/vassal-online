...
public class BrowserHelpFile extends AbstractBuildable implements Configurable {
  ...
  /**
   * Extract the HTML from the module, or create an External URL from the Starting page if there is
   * none
   */
  protected void extractContents() {
    try (ZipInputStream in =
      ...
      logger.error(
          "File not found in data archive: {}", "help/" + getContentsResource(), e); // NON-NLS
      setFallbackUrl();
    } catch (IOException e) {
      logger.error(
          "Error while reading file {} from data archive",
          "help/" + getContentsResource(),
          e); // NON-NLS
      setFallbackUrl();
    }
  }
  ...
  protected String evaluateStartingPage() {
    try {
      ...
      final String stage1 =
          new FormattedStringExpression(startingPage)
              .evaluate(GameModule.getGameModule(), this, trail);
      // Evaluate the resulting expression
      return Expression.createExpression(stage1).evaluate(GameModule.getGameModule(), this, trail);
    } catch (ExpressionException e) {
      // Not something that is going to happen often and indicates a broken module setup, so report
      // it properly.
      ErrorDialog.dataWarning(
          new BadDataReport(
              Resources.getString("Error.expression_error"),
              "Expression=" + startingPage + ", Error=" + e.getError(),
              e));
    }
    return startingPage;
  }
  ...
  public HelpFile getHelpFile() {
    return HelpFile.getReferenceManualPage(
        "HelpMenu.html", "HtmlHelpFile"); // $NON-NLS-1$ //$NON-NLS-2$
  }
  ...
  /**
   * The attributes we want to expose in the editor are not the same as the ones we want to save to
   * the buildFile, so we use this object to specify the properties in the editor. Also packs up the
   * contents directory and saves it to the ArchiveWriter
   */
  protected class ConfigSupport implements AutoConfigurable {
    ...
    public Class<?>[] getAttributeTypes() {
      return new Class<?>[] {String.class, ContentsConfig.class, StartPageConfig.class};
    }
  ...
  /**
   * Handles the packaging of the target directory into the module file after the user saves the
    ...
    if (myI18nData == null) {
      myI18nData =
          new ComponentI18nData(
              this,
              "BrowserHelpFile." + getConfigureName(),
              null, // NON-NLS
              new String[] {TITLE},
              new boolean[] {true},
