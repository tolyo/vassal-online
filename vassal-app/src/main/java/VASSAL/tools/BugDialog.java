...
public class BugDialog extends JDialog {
  ...
  public BugDialog(Frame owner, Throwable thrown, String key, String resource) {
    ...
    if (resource == null) {
      header =
          new JXHeader(
              Resources.getString(key + ".heading"),
              Resources.getString(key + ".message")); // NON-NLS
    } else {
      if ("".equals(resource)) {
      ...
      header =
          new JXHeader(
              Resources.getString(key + ".heading"),
              Resources.getString(key + ".message"),
              new ImageIcon(BugDialog.class.getResource(resource))); // NON-NLS
    }
    ...
    addWindowListener(
        new WindowAdapter() {
          ...
          public void windowClosed(WindowEvent e) {
            if (checkRequest != null) checkRequest.cancel(true);

            if (sendRequest != null) sendRequest.cancel(true);
          }
        });
  ...
  private Component buildContentsPanel() {
    ...
    contents.add(
        buildNonReportingVersionPanel(key + ".old_version_instructions"),
        "oldVersionPanel"); // NON-NLS
    contents.add(
        buildNonReportingVersionPanel(key + ".test_version_instructions"),
        "testVersionPanel"); // NON-NLS
  ...
  private Component buildVersionCheckPanel() {
    ...
    final JPanel panel = new JPanel(new MigLayout("", "", "[]push[]push")); // NON-NLS
  ...
  private Component buildCurrentVersionPanel() {
    final FlowLabel label =
        new FlowLabel(Resources.getString(key + ".current_version_instructions")); // NON-NLS
    ...
    final JLabel emailLabel = new JLabel(Resources.getString("BugDialog.user_email_address"));
    ...
    final JPanel panel =
        new JPanel(new MigLayout("hidemode 3", "", "[]unrel[]rel[]unrel[]unrel[]rel[]")); // NON-NLS
    panel.add(label, "cell 0 0, growx, pushx"); // NON-NLS
  ...
  private Component buildNonReportingVersionPanel(String key) {
    ...
    final JPanel panel = new JPanel(new MigLayout("hidemode 3", "", "[]unrel[]rel[]")); // NON-NLS
    panel.add(label, "cell 0 0, growx, pushx"); // NON-NLS
  ...
  private Component buildEmergencySavePanel() {
    final FlowLabel label = new FlowLabel(Resources.getString(key + ".how_to_proceed")); // NON-NLS
  ...
  private Component buildSendingBugReportPanel() {
    ...
    final FlowLabel label = new FlowLabel(Resources.getString("BugDialog.sending_bug_report"));
  ...
  private class SendRequest extends SwingWorker<Void, Void> {
    ...
    protected Void doInBackground() throws Exception {
      ...
      // Make the request to the server and wait for the latch.
      BugUtils.sendBugReport(emailField.getText(), descriptionArea.getText(), errorLog, thrown);
