...
public class PlaySound extends Decorator implements TranslatablePiece {
      ...
      if (!gm.getGameState().isFastForwarding()) {
        if (noSuppress || !gm.isSuppressSounds()) {
          final AuditTrail audit =
              AuditTrail.create(this, format, Resources.getString("Editor.PlaySound.sound_clip"));
          ...
          try {
            ...
            if (!GlobalOptions.getInstance().isSoundGlobalMute()) {
              final AudioClip clip =
                  GameModule.getGameModule().getDataArchive().getCachedAudioClip(clipName);
              if (clip != null) {
                clip.play();
              }
            }
            ...
            reportDataError(
                this,
                Resources.getString("Error.not_found", "Audio Clip"),
                "Clip=" + clipName,
  ...
  public String getDescription() {
    String s =
        buildDescription("Editor.PlaySound.trait_description", format.getFormat(), description);
  ...
  public static class Ed implements PieceEditor {
    ...
    public Ed(PlaySound p) {
      ...
      soundConfig =
          new AudioClipConfigurer(null, "", GameModule.getGameModule().getArchiveWriter(), p);
    ...
    public String getType() {
      final SequenceEncoder se = new SequenceEncoder(';');
      se.append(soundConfig.getValueString())
          .append(menuConfig.getValueString())
          .append(keyConfig.getValueString())
          .append(sendConfig.getValueString())
