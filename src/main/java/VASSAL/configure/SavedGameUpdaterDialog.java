...
public class SavedGameUpdaterDialog extends JDialog {
  ...
  private void initComponents() {
    ...
    versionField.setMaximumSize(
        new Dimension(versionField.getMaximumSize().width, versionField.getPreferredSize().height));
    versionBox.add(versionField);
    final JButton importButton =
        new JButton(Resources.getString("Editor.SavedGameUpdaterDialog.import_gamepiece"));
    importButton.addActionListener(e -> importPieceInfo());
    versionBox.add(importButton);
    add(versionBox);
    final JButton exportButton =
        new JButton(Resources.getString("Editor.SavedGameUpdaterDialog.export_gamepiece"));
    ...
    final JButton chooseGamesButton =
        new JButton(Resources.getString("Editor.SavedGameUpdaterDialog.choose"));
    ...
    savedGamesList.setCellRenderer(
    ...
    final JButton helpButton =
        new JButton(Resources.getString("Editor.SavedGameUpdaterDialog.help"));
    ...
    final JButton closeButton =
        new JButton(Resources.getString("Editor.SavedGameUpdaterDialog.close"));
          ...
          for (int i = 0; i < n; ++i) {
            try {
              ...
              GameModule.getGameModule()
                  .warn(
                      Resources.getString(
                          "Editor.SavedGameUpdaterDialog.updated_message",
                          savedGame.getName(),
  ...
  private void exportPieceInfo() {
    ...
    if (JFileChooser.CANCEL_OPTION != fc.showSaveDialog(this)) {
      ...
      try (OutputStream fout = Files.newOutputStream(fc.getSelectedFile().toPath());
        ...
        showErrorMessage(
            e,
            Resources.getString("Editor.SavedGameUpdaterDialog.ex_fail"),
            Resources.getString("Editor.SavedGameUpdaterDialog.ex_unable"));
      }
    }
  }
    ...
    if (JFileChooser.CANCEL_OPTION != fc.showOpenDialog(this)) {
      ...
      try (InputStream fin = Files.newInputStream(fc.getSelectedFile().toPath());
        ...
        if (!GameModule.getGameModule().getGameName().equals(moduleName)) {
          showErrorMessage(
              null,
              Resources.getString("Editor.SavedGameUpdaterDialog.im_fail"),
              Resources.getString("Editor.SavedGameUpdaterDialog.im_wrong", moduleName));
          oldPieceInfo = null;
          ...
          showErrorMessage(
              null,
              Resources.getString("Editor.SavedGameUpdaterDialog.im_fail"),
              Resources.getString("Editor.SavedGameUpdaterDialog.im_current", moduleVersion));
          oldPieceInfo = null;
          versionField.setText(null);
        } else {
      ...
      // FIXME: review error message
        ...
        showErrorMessage(
            e,
            Resources.getString("Editor.SavedGameUpdaterDialog.im_fail"),
            Resources.getString("Editor.SavedGameUpdaterDialog.im_unable"));
        oldPieceInfo = null;
      } catch (IllegalArgumentException e) { // catches malformed input files
        showErrorMessage(
            e,
            Resources.getString("Editor.SavedGameUpdaterDialog.im_fail"),
            Resources.getString("Editor.SavedGameUpdaterDialog.im_malformed"));
        oldPieceInfo = null;
      }
    }
