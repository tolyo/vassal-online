...
public interface Properties {
  ...
  /**
   * In order to maintain Legacy support after the fix of bug 12951, we need to save the Obscured
   * state prior to cards being drawn, let it be changed in the draw process, then restore it
   * afterwards. Don't ask.
   */
  String OBSCURED_BY_PRE_DRAW = "ObscuredPreDraw"; // NON-NLS

  /**
   * If non-null, then return an instance of {@link EventFilter}. The piece will not respond to
   * band-select events if the filter rejects them
   */
  String BAND_SELECT_EVENT_FILTER = "bandSelectEventFilter"; // NON-NLS

  /**
   * If non-null, then return an instance of {@link EventFilter}. The piece will not respond to
   * selection events if the filter rejects them
   */
  String SELECT_EVENT_FILTER = "selectEventFilter"; // NON-NLS

  /**
   * If non-null, then return an instance of {@link EventFilter}. The piece will not respond to move
   * events if the filter rejects them
   */
  ...
  /**
   * Return a KeyCommand[] object representing the popup menu equivalencies for the key commands
   * recognized by this piece
   */
  ...
  /**
   * If Boolean.TRUE, then treat the piece as if it were not rotated. This effects the value
   * returned by {@link GamePiece#getShape} and the way the piece is drawn.
   *
   * @see FreeRotator
   */
  ...
  /**
   * Return a String representing the visible features of the piece. If this String changes value,
   * then the piece should be refreshed
   */
