 ...
 *
 * <p>This Module-level component manages a set of one or more {@link ChessClock} clocks. It creates
 * a master "Chess Clocks" button on the Module's toolbar. Clicking the button shows/hides the
 * clocks themselves, when no clocks are running, or stops all clocks if one is running. The Chess
 * Clock Control component also contains the configurable elements that specify how the clocks are
 * to be displayed.
 */
public class ChessClockControl extends AbstractConfigurable
  ...
  protected static final UniqueIdManager ID_MGR =
      new UniqueIdManager("ChessClockControl"); // $NON-NLS-1$

  protected List<ChessClock> chessclocks =
      new ArrayList<>(); // List of individual chess clocks attached to our control
  ...
  protected boolean
      chessClocksVisible; // Whether the individual clocks are visible or hidden (as opposed to just
  // main chess clock button)
  protected boolean
      onlineGame; // If we've ever detected a live "online" connection during use of the clocks
  ...
  protected boolean
      instanceIsActive; // True when this instance has been fully initialized & registered (and not
  // yet shut down)
  ...
  public static final String CHESSMENU_START_OPPONENT =
      Resources.getString("ChessClock.startOpponent"); // NON-NLS
  ...
  public ChessClockControl() {
    ...
    chessClockButton =
        new LaunchButton(
            "Chess Clocks", BUTTON_TOOLTIP, BUTTON_TEXT, SHOW_HOTKEY, ICON, al); // $NON-NLS-1$
  ...
  /**
   * Builds the clock control's component hierarchy from a given XML element, or a null one is given
   * initializes a brand new default "new clock control" hierarchy.
   *
  ...
  public void build(Element e) {
    ...
    // When creating a brand new Chess Clock Control, start with one chess clock for each "non solo"
    // side in the roster.
  ...
  public void addTo(Buildable parent) {
    ...
    // BR// We handle adding our chessclocks to the toolbar during initial build, because otherwise
    // their buttons
    // BR// would annoyingly appear before ours, because of the awkward post-order behavior of
    // build.
   ...
   *
   * @param parent - Presumably the main GameModule, but in any event that's what we remove
   *     ourselves from.
   */
  @Override
  ...
  /**
   * @return List of valid subcomponent class types for this component (in our case, individual
   *     Chess Clocks!) which can be added in the Editor.
   */
  @Override
  ...
  public static ChessClockControl getInstance() {
    return GameModule.getGameModule().getAllDescendantComponentsOf(ChessClockControl.class).stream()
        .findFirst()
        .orElse(null);
  }
  ...
  /**
   * @return a command to start the opponents clock after stopping the one currently running (if
   *     any)
   */
  public Command startOpponentClock() {
    ...
    if (chessclocks.isEmpty()
        || chessclocks.size() != 2
        || mySide == null
        || mySide.contains("observer")) {
      return command;
    }

    boolean clockTicking = false;
    for (final ChessClock clock : chessclocks) {
      // We want to start exactly one clock, which doesn't belong to us, and only if it isn't
      // already started
      if (!clockTicking && !mySide.equals(clock.getSide())) {
        ...
        clockTicking = true;
          ...
          && (mySide.equals(clock.getSide()) || clockTicking)) { // If our clock is ticking, stop it
        command = command.append(clock.updateState(false));
      }
    }
  ...
  /**
   * @return a command to start the requested clock after stopping the one currently running (if
   *     any)
   */
   ...
   * Does one of the following, in order of priority: (1) If the clocks are hidden, show them (2) If
   * any clock is running, stop them all (3) Hide all the clocks
   */
  ...
  /**
   * Tells Chess Clock Control that this is a real online game, meaning we move to stricter
   * verification standards for clocks (completely non-online clocks are really just "sandbox toys")
   *
  ...
  public static class IconConfig implements ConfigurerFactory {
    ...
    public Configurer getConfigurer(AutoConfigurable c, String key, String name) {
      return new IconConfigurer(
          key, name, ((ChessClockControl) c).chessClockButton.getAttributeValueString(ICON));
    }
  }
  ...
  public static class TimeStyleConfig extends TranslatableStringEnum {
    ...
    public String[] getI18nKeys(AutoConfigurable target) {
      return new String[] {
        "Editor.ChessClock.always", "Editor.ChessClock.auto", "Editor.ChessClock.never"
      };
    }
  }
  ...
  /**
   * Our "command" format for passing information about the master clock control between computers
   * (or to/from save and log files)
   */
  private class UpdateClockControlCommand extends Command {
    ...
    /**
     * Process a master clock button update. (1) Update visibility status as appropriate (2) Check
    ...
    void buildPopup() {
      ...
      if (getClocksTicking() > 0) {
        s =
            CHESSMENU_PAUSE
                + "  "
                + NamedHotKeyConfigurer.getString(pauseListener.getNamedKeyStroke());
      } else {
        if (getChessClocksVisible()) {
          s =
              CHESSMENU_HIDE
                  + "  "
                  + NamedHotKeyConfigurer.getString(showListener.getNamedKeyStroke());
        } else {
          s =
              CHESSMENU_SHOW
                  + "  "
      ...
      item =
          new JMenuItem(
              CHESSMENU_NEXT
                  + "  "
                  + NamedHotKeyConfigurer.getString(nextListener.getNamedKeyStroke()));
      item.addActionListener(this);
      popup.add(item);
      ...
      item =
          new JMenuItem(
              CHESSMENU_START_OPPONENT
                  + "  "
                  + NamedHotKeyConfigurer.getString(startOpponentListener.getNamedKeyStroke()));
      item.addActionListener(this);
      popup.add(item);
    }
