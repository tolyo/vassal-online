...
public abstract class Launcher {
  ...
  protected Launcher(String[] args) {
    ...
    final StartUp start = SystemUtils.IS_OS_MAC ? new MacOSXStartUp() : new StartUp();
    ...
    final SimpleRunnableFuture<Void> fut =
        new SimpleRunnableFuture() {
          @Override
          public void run() {
            try {
              launch();
    ...
    try {
      ...
      if (t instanceof OutOfMemoryError) {
        ...
        try (Prefs p = new Prefs(Prefs.getGlobalPrefs().getEditor(), gname)) {
          final IntConfigurer maxHeapConf = new IntConfigurer(GlobalOptions.MAXIMUM_HEAP, "", 1024);
        ...
        System.exit(2);
      } else if (t instanceof ExtensionsLoader.LoadExtensionException || t instanceof IOException) {
        FutureUtils.wait(
            ErrorDialog.showDetails(
                e,
                ThrowableUtils.getStackTrace(e),
  ...
  private static final List<LoadConstraint> constraints =
      List.of(
          new LoadConstraint(
              "VASL", "3.4", "", "<a href=\"https://vasl.info\">vasl.info</a>"), // NON-NLS
          new LoadConstraint(
              "VSQL",
              "3.4",
              "",
              "<a href=\"https://vassalengine.org/wiki/Module:Squad_Leader\">vassalengine.org</a>"), // NON-NLS
          new LoadConstraint(
              "Paths of Glory",
              "",
              "9.9",
              "<a href=\"https://vassalengine.org/wiki/Module:Paths_of_Glory\">vassalengine.org</a>") // NON-NLS
          );
  ...
  public static boolean checkModuleLoadable(ModuleMetaData md) {
    ...
    for (final LoadConstraint rule : constraints) {
      if (rule.moduleName.equals(name)) {
        if (!rule.minVassalVersion.isEmpty()
            && (VersionUtils.compareVersions(md.getVassalVersion(), rule.minVassalVersion) < 0)) {
          ErrorDialog.show(
              "Error.module_too_old_vassal_version",
              name,
              Info.getVersion(),
              rule.newVersionLink); // NON-NLS
          return false;
        }
        if (!rule.minModuleVersion.isEmpty()
            && (VersionUtils.compareVersions(md.getVersion(), rule.minModuleVersion) < 0)) {
          ErrorDialog.show(
              "Error.module_too_old_module_version",
              name,
              Info.getVersion(),
              rule.newVersionLink); // NON-NLS
          return false;
