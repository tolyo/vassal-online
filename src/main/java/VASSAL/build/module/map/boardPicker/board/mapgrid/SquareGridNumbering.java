...
public class SquareGridNumbering extends RegularGridNumbering {
  ...
  public void forceDraw(
    ...
    final int minCol =
        reversed
            ? (int)
                Math.ceil(
                    (bounds.x - scale * grid.getOrigin().x + bounds.width - region.x) / deltaX)
            : (int) Math.floor((region.x - bounds.x - scale * grid.getOrigin().x) / deltaX);
    final double xmin =
        reversed
            ? bounds.x - scale * grid.getOrigin().x + bounds.width - deltaX * minCol
    ...
    final int minRow =
        reversed
            ? (int)
                Math.ceil(
                    (bounds.y - scale * grid.getOrigin().y + bounds.height - region.y) / deltaY)
            : (int) Math.floor((region.y - bounds.y - scale * grid.getOrigin().y) / deltaY);
    final double ymin =
        reversed
            ? bounds.y - scale * grid.getOrigin().y + bounds.height - deltaY * minRow
  ...
  public Point getCenterPoint(int col, int row) {
    ...
    return new Point(
        (int) (col * grid.getDx() + grid.getOrigin().x),
