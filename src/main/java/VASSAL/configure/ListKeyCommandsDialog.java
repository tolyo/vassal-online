...
/**
 * Editor tool for finding all the key commands (and named key commands) in a module, and applying
 * sorts/filters to them.
 */
public class ListKeyCommandsDialog extends JDialog {
  ...
  public ListKeyCommandsDialog(
    ...
    final NoInsetButton clear =
        new NoInsetButton("no", IconFamily.XSMALL, "Editor.clear"); // NON-NLS
  ...
  public class JumpAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      final AbstractConfigurable jumpTo =
          ((MyTableModel) (table.getModel())).getSourceFor(rowIndex);
  ...
  /**
   * For a given search target (either an AbstractConfigurable or a trait(Decorator) of a game
   * piece), pull out its list of key commands and add them to our table's list.
   *
   * @param target search target to check
   * @param list our table's list of column strings to be appended to
   * @param configurable If search target is a trait, this is the original AbstractConfigurable
   *     (e.g. a PrototypeDefinition, a Piece Slot, etc) that it came from.
   */
  private static void checkSearchTarget(
      SearchTarget target,
      List<Pair<String[], AbstractConfigurable>> list,
      ...
      for (final NamedKeyStroke k : keys) {
        if (k != null) {
          ...
          if (!StringUtils.isEmpty(cmd_key) || !StringUtils.isEmpty(cmd_name)) {
            ...
            // Depending on whether this is an AbstractConfigurable (i.e. Editor component) or part
            // of a GamePiece, pull out appropriate descriptor fields.
            if (target instanceof AbstractConfigurable) {
              ...
              if (configurable instanceof PrototypeDefinition) {
                src_name =
                    Resources.getString("Editor.ListKeyCommands.prototype")
                        + ": "
            ...
            list.add(
                Pair.of(
                    new String[] {cmd_key, cmd_name, src_type, src_name, src_path, src_desc},
                    configurable));
          }
        }
      }
  ...
  /**
   * For a given AbstractConfigurable, determine if it is one of the "Game Piece" types. If it IS,
   * parse each individual Decorator to check as a search target. Otherwise, for non-game-piece
   * configurables, just check the configurable itself.
   *
   * @param target AbstractConfigurable to check
   * @param list Our table list to which to add any key commands found
   */
  private static void checkForKeyCommands(
      AbstractConfigurable target, List<Pair<String[], AbstractConfigurable>> list) {
    GamePiece p;
    boolean protoskip;
  ...
  /**
   * Recursively scans the module for key commands and adds appropriate entries to our table's list
   * of key commands
   *
   * @param target current target component
   * @param list our table's list of column strings, to which entries will be appended
   */
  private static void recursivelyFindKeyCommands(
      AbstractBuildable target, List<Pair<String[], AbstractConfigurable>> list) {
