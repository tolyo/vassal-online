...
public class EchoClient implements Runnable, PropertyChangeListener {
  ...
  public synchronized void showCHAT(PeerInfo pPeerInfo, String msg) {
    ...
    if (!sender.getName().startsWith(NAME) && msg.startsWith("CHAT")) { // $NON-NLS-1$
      msg =
          "<"
              + client.getUserInfo().getName() // $NON-NLS-1$
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    if (log != null && evt.getPropertyName().equals(ChatServerConnection.AVAILABLE_ROOMS)) {
      try {
        log.write("----------" + (new Date()) + "---------\n"); // $NON-NLS-1$ //$NON-NLS-2$
