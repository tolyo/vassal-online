...
/**
 * General-purpose condition indicating that VASSAL has encountered data that's inconsistent with
 * the current module. A typical example would be failing to find a map/board/image/prototype from
 * the supplied name. Covers a variety of situations where the most likely cause is a module version
 * compatibility issue.
 *
 * <p>This is for recoverable errors that occur during game play, as opposed to {@link
 * IllegalBuildException}, which covers errors when building a module
 *
...
public class BadDataReport {
  ...
  /** Data about the cause of the error. Usually the expression that causes the problem */
  ...
  /** Owning component or trait that generated the error */
  ...
  /**
   * BadDataReport for a general exception raised during Expression Execution with supplied audit
   * info
   *
   * @param message Message to display
   * @param data Data causing error
   ...
   */
  public BadDataReport(
      String message, String data, Throwable cause, Auditable owner, AuditTrail auditTrail) {
    this(null, null, message, data, cause);
    this.owner = owner;
  ...
  /**
   * Expanded Bad Data Report called by Traits. Display additional information to aid debugging
   *
   * <p>NB. Note use of piece.getLocalizedName() rather than
   * Decorator.getOuterMost().getLocalizedName() which can result in infinite BadData reporting
   * loops.
   *
  ...
  public BadDataReport(
      String pieceName, String traitDesc, String message, String data, Throwable cause) {
    String m = "";
  ...
  /**
   * Return the name of the piece. For Decorators, return the name of the inner piece as the Bad
   * Data Report may have been generated by the call to get the name of the Decorator in the first
   * place.
   *
   * @param piece
   * @return
  ...
  /**
   * Expanded Bad Data Report for AbstractConfigurables. Display the name and type of the
   * Configurable
   *
  ...
  public BadDataReport(
    ...
    this.message =
        c.getConfigureName()
            + "["
            + ConfigureTree.getConfigureName(c.getClass())
            + "]: "
            + message
            + " "
            + getAuditMessage();
  ...
  private String getAuditMessage() {
    return additionalInfo
        ? Resources.getString(
            AuditTrail.isEnabled() ? "BadDataReport.see_errorlog" : "BadDataReport.enable_pref")
        : "";
  }
