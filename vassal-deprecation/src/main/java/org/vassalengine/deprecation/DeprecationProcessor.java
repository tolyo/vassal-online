...
public final class DeprecationProcessor extends AbstractProcessor {
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    for (TypeElement annotationType : annotations) {
      ...
      final Map<Boolean, List<Element>> partitionByValidity =
          elements.stream()
              .collect(
                  Collectors.partitioningBy(
                      element -> isValidDate(element.getAnnotation(Deprecated.class).since())));

      partitionByValidity.get(Boolean.TRUE).stream()
          .filter(
              element ->
                  hasYearPassedSinceDeprecation(element.getAnnotation(Deprecated.class).since()))
          .forEach(
              element ->
                  processingEnv
                      .getMessager()
                      .printMessage(
                          ...
                          buildMessage(
                              element,
                              "Element has been deprecated for over a year and is due for removal")));
    }
  ...
  private static String buildMessage(Element element, String message) {
    ...
    if (!element.getKind().isClass()) {
      sb.append(element.getEnclosingElement()).append(".");
    }

    sb.append(element.toString()).append(": ").append(message);
