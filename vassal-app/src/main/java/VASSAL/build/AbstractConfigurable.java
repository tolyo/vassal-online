...
/**
 * An abstract implementation of the {@link Configurable} interface. To make a component which is
 * both buildable from the buildFile (XML) and whose XML attributes are then editable/configurable
 * with a dialog in the Editor, extend this class. Takes care of most of the Configurable
 * functionality. Provides the basis for a component to have a configuration dialog in the Editor,
 * allowing various attributes to be edited, retained, and saved/loaded from the module's buildFile
 * (XML).
 */
public abstract class AbstractConfigurable extends AbstractBuildable
    implements AutoConfigurable, SearchTarget {
  protected PropertyChangeSupport changeSupport;
  protected String
      name; // Language-independent name used for programmatic identification (including within
  // Modules by e.g. Traits and module components)
  ...
  /**
   * ConfigureName is used, in particular, by the Editor to track the names of components, and is
   * preferred by {@link VASSAL.tools.UniqueIdManager}.
   *
   * @return The language-independent name of this component used for programmatic identification
   *     (including within Modules by e.g. Traits and module components)
   */
  @Override
   ...
   * @return a {@link VisibilityCondition} for this attribute, or null if attribute should always be
   *     visible.
   */
  @Override
  ...
  /**
   * @return Generate a standard prefix for i18n keys for attributes of this component -
  ...
  /**
   * Over-ride the default attribute translatability. This is called by individual components to
   * force specific attributes to be translatable or not translatable
   *
  ...
  /**
   * @return A list of all child components (i.e. subcomponents) of this component that are
   *     configurable.
   */
  @Override
  ...
  /**
   * The default {@link Configurer} of an {@link AbstractConfigurable} class is an instance of
   * {@link AutoConfigurer}
   *
  ...
  public Configurer getConfigurer() {
    if (config == null) {
      ...
      // Some Configurers clear their configurer during a reset to ensure a new one is generated to
      // reset any internal components
      if (config == null) {
        config = new AutoConfigurer(this);
      }
    }
  ...
  /**
   * Returns the name of the configurable type for display purposes. Reflection is used to call
   * <code>getConfigureTypeName()</code>, which should be a static method if it exists in the given
   * class. (This is necessary because static methods are not permitted in interfaces.)
   *
